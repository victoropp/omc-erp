...
exports.generateDefaultMockData = void 0;
exports.useAnalyticsData = useAnalyticsData;
const react_1 = require("react");
const api_1 = require("@/services/api");
const react_hot_toast_1 = require("react-hot-toast");
function useAnalyticsData({ endpoint, filters, realTimeUpdates = false, autoRefresh, mockDataGenerator, }) {
    ...
    const fetchAnalyticsData = async (showLoading = true) => {
        try {
            ...
            let analyticsData;
            try {
                const response = await api_1.analyticsService.get(`${endpoint}?${params.toString()}`);
                ...
                if (mockDataGenerator) {
                    ...
                    analyticsData = (0, exports.generateDefaultMockData)(filters.period);
                }
            }
            ...
            if (mockDataGenerator) {
                ...
                setData((0, exports.generateDefaultMockData)(filters.period));
            }
        }
        finally {
            setLoading(false);
    ...
    const exportData = async (format) => {
        try {
            react_hot_toast_1.toast.loading(`Exporting analytics as ${format.toUpperCase()}...`);
            const params = new URLSearchParams();
            Object.entries(filters).forEach(([key, value]) => {
                if (value && value !== 'all') {
                    params.append(key, value);
            ...
            const response = await api_1.analyticsService.get(`${endpoint}/export?${params.toString()}`, {
                responseType: 'blob'
            });
...
exports.generateDefaultMockData = generateDefaultMockData;
...
const generateTrendData = (period) => {
    ...
    const labels = Array.from({ length: dataPoints }, (_, i) => period === '7d' ? `Day ${i + 1}` :
        period === '30d' ? `${i + 1}` :
            period === '90d' ? `Week ${i + 1}` :
                `Month ${i + 1}`);
    return {
