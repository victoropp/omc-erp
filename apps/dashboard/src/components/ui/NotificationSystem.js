...
function NotificationSystem({ maxNotifications = 50, autoHideAfter = 5, enableWebSocket = true, }) {
    ...
    const handleWebSocketMessage = (0, react_1.useCallback)((data) => {
        switch (data.type) {
    ...
    (0, react_1.useEffect)(() => {
        if (enableWebSocket) {
            ...
            // Add message handler
            const originalHandler = api_1.wsService['handleMessage'];
            api_1.wsService['handleMessage'] = (data) => {
                originalHandler.call(api_1.wsService, data);
    ...
    return (<div className="relative">
      {/* Notification Bell */}
      <button onClick={() => setIsOpen(!isOpen)} className={`relative p-2 rounded-lg transition-colors ${actualTheme === 'dark'
        ...
        {unreadCount > 0 && (<ui_1.Badge variant="danger" className="absolute -top-1 -right-1 text-xs">
      ...
      <framer_motion_1.AnimatePresence>
        {isOpen && (<framer_motion_1.motion.div initial={{ opacity: 0, scale: 0.95, y: -10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -10 }} transition={{ duration: 0.2 }} className={`absolute right-0 top-full mt-2 w-96 rounded-xl shadow-2xl border z-50 ${actualTheme === 'dark'
            ...
            <div className="p-4 border-b border-white/10">
              <div className="flex items-center justify-between">
                ...
                <div className="flex space-x-2">
                  {unreadCount > 0 && (<ui_1.Button variant="ghost" size="sm" onClick={markAllAsRead}>
                      Mark all read
                    </ui_1.Button>)}
                  <ui_1.Button variant="ghost" size="sm" onClick={clearAll}>
                    Clear all
                  </ui_1.Button>
                </div>
              </div>
            ...
            <div className="max-h-96 overflow-y-auto">
              {notifications.length === 0 ? (<div className="p-8 text-center">
                  ...
                  {notifications.map((notification) => (<framer_motion_1.motion.div key={notification.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className={`p-4 relative border-l-4 ${getBorderColor(notification.type)} ${!notification.read
                        ? actualTheme === 'dark' ? 'bg-white/5' : 'bg-gray-50'
                        : ''} hover:${actualTheme === 'dark' ? 'bg-white/5' : 'bg-gray-50'} transition-colors cursor-pointer`} onClick={() => markAsRead(notification.id)}>
                      <div className="flex items-start space-x-3">
                        ...
                        clearNotification(notification.id);
                          ...
                          <div className="flex items-center justify-between mt-2">
                            ...
                            {notification.actionable && notification.action && (<ui_1.Button variant="ghost" size="sm" onClick={(e) => {
                            e.stopPropagation();
