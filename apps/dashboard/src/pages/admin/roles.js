...
const RoleManagement = () => {
    ...
    const [isPermissionModalOpen, setIsPermissionModalOpen] = (0, react_1.useState)(false);
    ...
    const loadData = async () => {
        try {
            ...
            react_hot_toast_1.toast.error('Failed to load roles and permissions');
        }
        finally {
            setLoading(false);
        }
    ...
    const handleUpdateRole = async (e) => {
        e.preventDefault();
        try {
            react_hot_toast_1.toast.success('Role updated successfully');
            setIsRoleModalOpen(false);
            resetRoleForm();
            loadData();
        }
        catch (error) {
            react_hot_toast_1.toast.error('Failed to update role');
        }
    };
    const handleDeleteRole = async (roleId) => {
        if (confirm('Are you sure you want to delete this role?')) {
            try {
                react_hot_toast_1.toast.success('Role deleted successfully');
                loadData();
            }
            catch (error) {
                react_hot_toast_1.toast.error('Failed to delete role');
            }
        }
    };
    const resetRoleForm = () => {
    ...
    const roleColumns = [
        ...
        { key: 'userCount', header: 'Users', width: '8%',
            render: (value) => (<span className="text-primary-400 font-medium">{value}</span>)
        },
        { key: 'permissions', header: 'Permissions', width: '15%',
            render: (value) => (<div className="flex items-center space-x-2">
          <span className="text-dark-400">{value.length}</span>
          <span className="text-xs text-dark-500">permissions</span>
        </div>)
        },
        { key: 'isSystem', header: 'Type', width: '10%',
            render: (value) => (<span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${value ? 'bg-purple-500/20 text-purple-400 border border-purple-500/30' :
        ...
        { key: 'id', header: 'Actions', width: '12%',
          ...
          <ui_1.Button variant="ghost" size="sm" onClick={() => openEditRole(row)} disabled={row.isSystem}>
            Edit
          </ui_1.Button>
          <ui_1.Button variant="ghost" size="sm" onClick={() => setIsPermissionModalOpen(true)}>
            Permissions
          </ui_1.Button>
          {!row.isSystem && (<ui_1.Button variant="ghost" size="sm" onClick={() => handleDeleteRole(value)} className="text-red-400 hover:text-red-300">
              Delete
            </ui_1.Button>)}
        </div>)
        },
      ...
      <div className="space-y-8">
        ...
        <framer_motion_1.motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex justify-between items-center">
          ...
          <div className="flex space-x-4">
            ...
            <ui_1.Button variant="primary" size="sm" onClick={() => setIsRoleModalOpen(true)}>
              <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {/* Main Content */}
        <ui_1.Card>
          <ui_1.CardHeader title="Role Management">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
              <ui_1.Input placeholder="Search roles..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
              <ui_1.Select options={statusOptions} value={statusFilter} onChange={setStatusFilter} placeholder="Filter by status"/>
              <ui_1.Select options={moduleOptions} value={moduleFilter} onChange={setModuleFilter} placeholder="Filter by module"/>
            </div>
          </ui_1.CardHeader>
          <ui_1.CardContent>
            <ui_1.Table data={filteredRoles} columns={roleColumns} loading={loading} pagination={{
            page: 1,
            limit: 10,
        ...
        <ui_1.FormModal isOpen={isRoleModalOpen} onClose={() => {
            setIsRoleModalOpen(false);
            resetRoleForm();
        }} onSubmit={isEditMode ? handleUpdateRole : handleCreateRole} title={isEditMode ? 'Edit Role' : 'Create New Role'} submitText={isEditMode ? 'Update Role' : 'Create Role'}>
          ...
          <ui_1.Select label="Role Color" options={colorOptions} value={roleFormData.color} onChange={(value) => setRoleFormData({ ...roleFormData, color: value })} required/>
          ...
          <div>
            ...
            <div className="space-y-4 max-h-60 overflow-y-auto border border-dark-600 rounded-lg p-4">
              {Object.entries(permissionsByModule).map(([module, modulePermissions]) => (<div key={module}>
                  <h4 className="font-semibold text-white mb-2 capitalize">
                    {module.replace('_', ' ')}
                  </h4>
                  <div className="grid grid-cols-1 gap-2 pl-4">
                    {modulePermissions.map((permission) => (<div key={permission.id} className="flex items-center space-x-2">
                        <input type="checkbox" id={permission.id} checked={roleFormData.permissions.includes(permission.id)} onChange={(e) => {
                    if (e.target.checked) {
                        setRoleFormData({
                            ...roleFormData,
        ...
        {/* Permissions Modal */}
        <ui_1.FormModal isOpen={isPermissionModalOpen} onClose={() => setIsPermissionModalOpen(false)} onSubmit={() => setIsPermissionModalOpen(false)} title="Role Permissions Detail" submitText="Close">
          <div className="space-y-4">
            {Object.entries(permissionsByModule).map(([module, modulePermissions]) => (<div key={module}>
                ...
                <div className="space-y-2 pl-4">
                  {modulePermissions.map((permission) => (<div key={permission.id} className="flex items-center justify-between py-2">
                      <div>
