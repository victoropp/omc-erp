...
const UserManagement = () => {
    ...
    // Modal states
    const [isUserModalOpen, setIsUserModalOpen] = (0, react_1.useState)(false);
    const [isRoleModalOpen, setIsRoleModalOpen] = (0, react_1.useState)(false);
    const [isEditMode, setIsEditMode] = (0, react_1.useState)(false);
    const [isPermissionsModalOpen, setIsPermissionsModalOpen] = (0, react_1.useState)(false);
    // Form data states
    ...
    const [roleFormData, setRoleFormData] = (0, react_1.useState)({
        name: '',
        displayName: '',
        description: '',
        permissions: [],
    ...
    const loadData = async () => {
        try {
            ...
            react_hot_toast_1.toast.error('Failed to load user data');
        }
        finally {
            setLoading(false);
        }
    ...
    const handleCreateUser = async (e) => {
        e.preventDefault();
        try {
            react_hot_toast_1.toast.success('User created successfully');
            if (userFormData.sendWelcomeEmail) {
                react_hot_toast_1.toast.success('Welcome email sent to user');
            }
            ...
            react_hot_toast_1.toast.error('Failed to create user');
        }
    };
    const handleUpdateUser = async (e) => {
        e.preventDefault();
        try {
            react_hot_toast_1.toast.success('User updated successfully');
            setIsUserModalOpen(false);
            resetUserForm();
            loadData();
        }
        catch (error) {
            react_hot_toast_1.toast.error('Failed to update user');
        }
    };
    const handleCreateRole = async (e) => {
        e.preventDefault();
        try {
            react_hot_toast_1.toast.success('Role created successfully');
            setIsRoleModalOpen(false);
            resetRoleForm();
            loadData();
        }
        catch (error) {
            react_hot_toast_1.toast.error('Failed to create role');
        }
    };
    const handleResetPassword = async (userId) => {
        try {
            react_hot_toast_1.toast.success('Password reset email sent to user');
        }
        catch (error) {
            react_hot_toast_1.toast.error('Failed to send password reset email');
        }
    };
    const handleToggleUserStatus = async (userId, isActive) => {
        try {
            react_hot_toast_1.toast.success(isActive ? 'User activated' : 'User deactivated');
            loadData();
        }
        catch (error) {
    ...
    // Table columns
    const userColumns = [
        { key: 'fullName', header: 'User', width: '20%', sortable: true,
          ...
          <div>
            ...
            {row.employeeNumber && (<p className="text-xs text-dark-500">{row.employeeNumber}</p>)}
          </div>
        </div>)
        },
        { key: 'username', header: 'Username', width: '12%', sortable: true },
        { key: 'role', header: 'Role', width: '12%', sortable: true,
            render: (value) => (<span className="inline-flex px-2 py-1 text-xs font-medium rounded-full bg-purple-500/20 text-purple-400 border border-purple-500/30">
        ...
        { key: 'lastLoginAt', header: 'Last Login', width: '12%',
            render: (value) => (<span className="text-sm text-dark-400">
          {value ? new Date(value).toLocaleDateString() : 'Never'}
        </span>)
        },
        { key: 'twoFactorEnabled', header: '2FA', width: '8%',
            render: (value) => (<span className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-full ${value ? 'bg-green-500/20 text-green-400 border border-green-500/30' :
                    'bg-gray-500/20 text-gray-400 border border-gray-500/30'}`}>
          {value ? 'Enabled' : 'Disabled'}
        </span>)
        },
        { key: 'id', header: 'Actions', width: '15%',
            render: (value, row) => (<div className="flex space-x-1">
          <ui_1.Button variant="ghost" size="sm" onClick={() => openEditUser(row)}>
            Edit
          </ui_1.Button>
          <ui_1.Button variant="ghost" size="sm" onClick={() => handleResetPassword(value)}>
            Reset PWD
          </ui_1.Button>
          <ui_1.Button variant="ghost" size="sm" onClick={() => handleToggleUserStatus(value, !row.isActive)}>
    ...
    const roleColumns = [
        { key: 'displayName', header: 'Role Name', width: '20%', sortable: true },
        { key: 'name', header: 'System Name', width: '15%', sortable: true },
        { key: 'description', header: 'Description', width: '35%' },
        { key: 'permissions', header: 'Permissions', width: '15%',
            render: (value) => (<span className="text-sm text-dark-400">{value.length} permissions</span>)
        },
    ...
    return (<FuturisticDashboardLayout_1.FuturisticDashboardLayout>
      <div className="space-y-8">
        ...
        <framer_motion_1.motion.div key={activeTab} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
          {activeTab === 'users' && (<ui_1.Card>
              <ui_1.CardHeader title="User Management">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-4">
                  <ui_1.Input placeholder="Search users..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
                  <ui_1.Select options={[{ value: '', label: 'All Roles' }, ...roleOptions]} value={roleFilter} onChange={setRoleFilter} placeholder="Filter by role"/>
                  <ui_1.Select options={statusOptions} value={statusFilter} onChange={setStatusFilter} placeholder="Filter by status"/>
                  <ui_1.Select options={[{ value: '', label: 'All Departments' }, ...departmentOptions]} value={departmentFilter} onChange={setDepartmentFilter} placeholder="Filter by department"/>
                </div>
              </ui_1.CardHeader>
              <ui_1.CardContent>
                <ui_1.Table data={filteredUsers} columns={userColumns} loading={loading} pagination={{
                page: 1,
                limit: 10,
          ...
          {activeTab === 'roles' && (<ui_1.Card>
              <ui_1.CardHeader title="Roles & Permissions"/>
              <ui_1.CardContent>
                <ui_1.Table data={roles} columns={roleColumns} loading={loading} pagination={{
                page: 1,
                limit: 10,
                total: roles.length,
                onPageChange: () => { },
        ...
        {/* User Modal */}
        <ui_1.FormModal isOpen={isUserModalOpen} onClose={() => {
            setIsUserModalOpen(false);
            resetUserForm();
        }} onSubmit={isEditMode ? handleUpdateUser : handleCreateUser} title={isEditMode ? 'Edit User' : 'Create New User'} submitText={isEditMode ? 'Update User' : 'Create User'}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <ui_1.Input label="First Name" placeholder="Enter first name" value={userFormData.firstName} onChange={(e) => setUserFormData({ ...userFormData, firstName: e.target.value })} required/>
            <ui_1.Input label="Last Name" placeholder="Enter last name" value={userFormData.lastName} onChange={(e) => setUserFormData({ ...userFormData, lastName: e.target.value })} required/>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <ui_1.Input label="Email" type="email" placeholder="user@omc.com.gh" value={userFormData.email} onChange={(e) => setUserFormData({ ...userFormData, email: e.target.value })} required/>
            <ui_1.Input label="Username" placeholder="unique.username" value={userFormData.username} onChange={(e) => setUserFormData({ ...userFormData, username: e.target.value })} required/>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <ui_1.Input label="Phone" placeholder="+233-XXX-XXX-XXX" value={userFormData.phone} onChange={(e) => setUserFormData({ ...userFormData, phone: e.target.value })} required/>
            <ui_1.Input label="Employee Number (Optional)" placeholder="EMP-2024-XXX" value={userFormData.employeeNumber} onChange={(e) => setUserFormData({ ...userFormData, employeeNumber: e.target.value })}/>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <ui_1.Input label="Job Title" placeholder="Enter job title" value={userFormData.jobTitle} onChange={(e) => setUserFormData({ ...userFormData, jobTitle: e.target.value })} required/>
            <ui_1.Select label="Department" options={departmentOptions} value={userFormData.department} onChange={(value) => setUserFormData({ ...userFormData, department: value })} required/>
          </div>

          <ui_1.Select label="Role" options={roleOptions} value={userFormData.role} onChange={(value) => setUserFormData({ ...userFormData, role: value })} required/>

          {!isEditMode && (<div className="flex items-center space-x-2">
              <input type="checkbox" id="sendWelcomeEmail" checked={userFormData.sendWelcomeEmail} onChange={(e) => setUserFormData({ ...userFormData, sendWelcomeEmail: e.target.checked })} className="rounded border-gray-300"/>
          ...
          <div className="flex items-center space-x-2">
            <input type="checkbox" id="isActive" checked={userFormData.isActive} onChange={(e) => setUserFormData({ ...userFormData, isActive: e.target.checked })} className="rounded border-gray-300"/>
        ...
        {/* Role Modal */}
        <ui_1.FormModal isOpen={isRoleModalOpen} onClose={() => {
            setIsRoleModalOpen(false);
            resetRoleForm();
        }} onSubmit={handleCreateRole} title="Create New Role" submitText="Create Role">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <ui_1.Input label="Display Name" placeholder="Station Manager" value={roleFormData.displayName} onChange={(e) => setRoleFormData({ ...roleFormData, displayName: e.target.value })} required/>
            <ui_1.Input label="System Name" placeholder="station_manager" value={roleFormData.name} onChange={(e) => setRoleFormData({ ...roleFormData, name: e.target.value.toLowerCase().replace(/\s+/g, '_') })} required/>
          </div>

          <ui_1.Input label="Description" placeholder="Describe the role's responsibilities" value={roleFormData.description} onChange={(e) => setRoleFormData({ ...roleFormData, description: e.target.value })} required/>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 max-h-60 overflow-y-auto border border-dark-600 rounded-lg p-4">
              {allPermissions.map((permission) => (<div key={permission} className="flex items-center space-x-2">
                  <input type="checkbox" id={permission} checked={roleFormData.permissions.includes(permission)} onChange={(e) => {
                if (e.target.checked) {
                    setRoleFormData({
                        ...roleFormData,
          ...
          <div className="flex items-center space-x-2">
            <input type="checkbox" id="roleIsActive" checked={roleFormData.isActive} onChange={(e) => setRoleFormData({ ...roleFormData, isActive: e.target.checked })} className="rounded border-gray-300"/>
