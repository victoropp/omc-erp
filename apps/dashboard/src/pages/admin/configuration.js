...
exports.default = SystemConfigurationPage;
...
function SystemConfigurationPage() {
    ...
    const [selectedModule, setSelectedModule] = (0, react_1.useState)('GENERAL');
    const [configurations, setConfigurations] = (0, react_1.useState)([]);
    const [loading, setLoading] = (0, react_1.useState)(true);
    const [saving, setSaving] = (0, react_1.useState)(false);
    const [editedConfigs, setEditedConfigs] = (0, react_1.useState)({});
    ...
    const loadConfigurations = async () => {
        try {
            ...
            react_hot_toast_1.toast.error('Failed to load configurations');
        }
        finally {
            setLoading(false);
        }
    };
    const handleConfigChange = (moduleKey, configKey, value) => {
    ...
    const handleSaveConfigurations = async () => {
        try {
            ...
            // });
            react_hot_toast_1.toast.success('Configurations saved successfully');
            setEditedConfigs({});
        }
        catch (error) {
            react_hot_toast_1.toast.error('Failed to save configurations');
        }
        finally {
            setSaving(false);
        }
    };
    const renderConfigInput = (module, config) => {
        const key = `${module}.${config.key}`;
        const value = editedConfigs[key] ?? config.value;
        switch (config.type) {
            case 'boolean':
                return (<framer_motion_1.motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => handleConfigChange(module, config.key, !value)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${value
                        ? 'bg-primary-500'
                        : actualTheme === 'dark' ? 'bg-dark-600' : 'bg-gray-300'}`}>
            <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${value ? 'translate-x-6' : 'translate-x-1'}`}/>
          </framer_motion_1.motion.button>);
            case 'number':
                return (<input type="number" value={value} onChange={(e) => handleConfigChange(module, config.key, Number(e.target.value))} min={config.validation?.min} max={config.validation?.max} className={`w-full px-3 py-2 rounded-lg border transition-colors ${actualTheme === 'dark'
                        ? 'bg-dark-700 border-dark-600 text-white'
                        : 'bg-white border-gray-300 text-gray-900'} focus:ring-2 focus:ring-primary-500`}/>);
            case 'select':
                return (<select value={value} onChange={(e) => handleConfigChange(module, config.key, e.target.value)} className={`w-full px-3 py-2 rounded-lg border transition-colors ${actualTheme === 'dark'
                        ? 'bg-dark-700 border-dark-600 text-white'
                        : 'bg-white border-gray-300 text-gray-900'} focus:ring-2 focus:ring-primary-500`}>
            {config.options?.map(option => (<option key={option.value} value={option.value}>
                {option.label}
              </option>))}
          </select>);
            case 'json':
                return (<textarea value={typeof value === 'string' ? value : JSON.stringify(value, null, 2)} onChange={(e) => {
                        try {
                    ...
                    }} rows={6} className={`w-full px-3 py-2 rounded-lg border font-mono text-sm transition-colors ${actualTheme === 'dark'
                        ? 'bg-dark-700 border-dark-600 text-white'
                        : 'bg-white border-gray-300 text-gray-900'} focus:ring-2 focus:ring-primary-500`}/>);
            default:
                return (<input type="text" value={value} onChange={(e) => handleConfigChange(module, config.key, e.target.value)} className={`w-full px-3 py-2 rounded-lg border transition-colors ${actualTheme === 'dark'
                        ? 'bg-dark-700 border-dark-600 text-white'
                        : 'bg-white border-gray-300 text-gray-900'} focus:ring-2 focus:ring-primary-500`}/>);
        }
    };
    ...
    return (<FuturisticDashboardLayout_1.FuturisticDashboardLayout>
      <div className="p-6">
        ...
        <div className="mb-8">
          ...
          <p className={`${actualTheme === 'dark' ? 'text-dark-400' : 'text-gray-600'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-1">
            <div className={`glass rounded-xl p-4 border ${actualTheme === 'dark' ? 'border-white/10' : 'border-gray-200'}`}>
              <h2 className={`text-lg font-semibold mb-4 ${actualTheme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                Configuration Modules
              </h2>
              
              <div className="space-y-2">
                {modules.map(module => (<framer_motion_1.motion.button key={module.name} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setSelectedModule(module.name)} className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg transition-all ${selectedModule === module.name
          ...
          <div className="lg:col-span-3">
            <div className={`glass rounded-xl p-6 border ${actualTheme === 'dark' ? 'border-white/10' : 'border-gray-200'}`}>
              {selectedModuleData && (<>
                  <div className="mb-6">
                  ...
                  <div className="space-y-6">
                    {selectedModuleData.configs.map(config => (<div key={config.key}>
                        <div className="flex items-center justify-between mb-2">
                          <label className={`font-medium ${actualTheme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                            {config.label}
                          </label>
                          {editedConfigs[`${selectedModule}.${config.key}`] !== undefined && (<span className="text-xs text-yellow-500">Modified</span>)}
                        </div>
                        
                        {config.description && (<p className={`text-sm mb-2 ${actualTheme === 'dark' ? 'text-dark-400' : 'text-gray-600'}`}>
                            {config.description}
                          </p>)}
                  ...
                  {/* Save Button */}
                  {Object.keys(editedConfigs).length > 0 && (<framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-8 flex justify-end space-x-4">
                      <button onClick={() => setEditedConfigs({})} className={`px-6 py-2 rounded-lg border transition-colors ${actualTheme === 'dark'
                    ? 'border-dark-500 text-dark-300 hover:bg-dark-700'
                    : 'border-gray-300 text-gray-700 hover:bg-gray-100'}`}>
                        Cancel
                      </button>
                      <framer_motion_1.motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleSaveConfigurations} disabled={saving} className="px-6 py-2 bg-gradient-primary text-white rounded-lg font-medium
