...
const BackupSettings: NextPage = () => {
  const [backupJobs, setBackupJobs] = useState<BackupJob[]>([]);
  const [backupFiles, setBackupFiles] = useState<BackupFile[]>([]);
  const [loading, setLoading] = useState(false);
  const [hasChanges, setHasChanges] = useState(false);
  const [activeTab, setActiveTab] = useState<'jobs' | 'files' | 'settings' | 'restore'>('jobs');
  ...
  const handleVerifyBackup = async (fileId: string) => {
    try {
      ...
      toast.success('Backup file verification successful');
    } catch (error) {
  ...
  // Sample data
  const sampleBackupJobs: BackupJob[] = [
    {
      id: '1',
          ...
          <Card>
            <CardContent className="p-6 text-center">
              ...
              <p className={`text-3xl font-bold mb-1 ${
                successfulBackups === backupJobs.length ? 'text-green-400' : 'text-yellow-400'
              }`}>
        ...
        >
                      ...
                      <div className="flex items-center space-x-3">
                        <div className="flex items-center space-x-2">
                          ...
                          <motion.button
                            ...
                            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                              job.enabled ? 'bg-primary-500' : 'bg-dark-600'
                            }`}
                          >
                            <span
                        ...
                        <div className="flex space-x-2">
                          <Button 
                            ...
                            disabled={job.status === 'running' || loading}
                          >
                    ...
                    <div key={file.id} className="flex items-center justify-between p-4 bg-dark-700 rounded-lg">
                      ...
                      <div className="flex space-x-2">
                        <Button 
                          ...
                          disabled={verifyingBackup === file.id}
                        >
          ...
          {activeTab === 'settings' && (
            <div className="space-y-6">
              <Card>
                <CardHeader title="General Backup Settings" />
                <CardContent className="space-y-6">
                  ...
                  <div>
                    ...
                    <Input defaultValue="/var/backups/omc-erp" placeholder="Enter backup path" />
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {[
                      ...
                      <div key={setting.key} className="flex items-center justify-between p-4 bg-dark-700 rounded-lg">
                        <div>
              ...
              <Card>
                <CardHeader title="Storage Destinations" />
                <CardContent>
                  <div className="space-y-4">
                    {[
                      ...
                      <div key={index} className="flex items-center justify-between p-4 bg-dark-700 rounded-lg">
                        <div className="flex items-center space-x-4">
                          <div className={`w-3 h-3 rounded-full ${
                            destination.status === 'connected' ? 'bg-green-400' : 'bg-red-400'
                          }`} />
                          <div>
          ...
          {activeTab === 'restore' && (
            <div className="space-y-6">
              ...
              <Card>
                <CardHeader title="Point-in-Time Recovery" />
                <CardContent>
                  <div className="space-y-4">
                    <p className="text-dark-400 text-sm">
