...
const UPPFClaimsManagement = () => {
    const [claims] = (0, react_1.useState)([
        {
            id: '1',
    ...
    const filteredClaims = (0, react_1.useMemo)(() => {
    ...
    const tableData = filteredClaims.map(claim => ({
        ...
        status: (<span className={`px-2 py-1 rounded-full text-xs font-medium border ${getStatusColor(claim.status)}`}>
        {getStatusText(claim.status)}
      </span>),
        submittedDate: claim.submittedDate,
        actions: (<div className="flex space-x-2">
        <Button_1.Button variant="outline" size="sm" onClick={() => handleViewClaim(claim)}>
          View
        </Button_1.Button>
    ...
    return (<FuturisticDashboardLayout_1.FuturisticDashboardLayout>
      <div className="space-y-8">
        ...
        <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
          <Card_1.Card className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              <div>
                <Input_1.Input placeholder="Search claims..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full"/>
              </div>
              <div>
        ...
        {/* Claim Detail Modal */}
        <Modal_1.Modal isOpen={showClaimModal} onClose={() => setShowClaimModal(false)} title="Claim Details">
          {selectedClaim && (<div className="space-y-6">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedClaim.documents.map((doc, index) => (<span key={index} className="px-3 py-1 bg-primary-500/20 text-primary-400 rounded-full text-sm border border-primary-500/30">
                      {doc}
                    </span>))}
                </div>
              </div>
              ...
              <div className="flex justify-end space-x-3 pt-4 border-t border-white/10">
                <Button_1.Button variant="outline" onClick={() => setShowClaimModal(false)}>
                  Close
                </Button_1.Button>
                {selectedClaim.status === 'pending' && (<>
                    <Button_1.Button variant="outline" className="text-red-400 border-red-400/30 hover:bg-red-400/10">
                      Reject
                    </Button_1.Button>
                    <Button_1.Button variant="primary">
                      Approve
