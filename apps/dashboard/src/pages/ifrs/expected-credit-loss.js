...
const ExpectedCreditLoss = () => {
    const [eclData] = (0, react_1.useState)([
        {
            id: '1',
    ...
    const tableData = eclData.map(item => ({
        ...
        stage: (<span className={`px-2 py-1 rounded text-xs font-medium ${getStageColor(item.stage)}`}>
        Stage {item.stage}
      </span>),
        parameters: (<div className="text-sm">
        <div>PD: {item.pd}%</div>
        <div>LGD: {item.lgd}%</div>
        <div>EAD: ₵{item.ead.toLocaleString()}</div>
      </div>),
        eclAmount: `₵${item.eclAmount.toLocaleString()}`,
        riskLevel: (<span className={`px-2 py-1 rounded-full text-xs font-medium border ${getRiskColor(item.riskLevel)}`}>
        {item.riskLevel.toUpperCase()}
      </span>),
        actions: (<div className="flex space-x-2">
        <Button_1.Button variant="outline" size="sm">Review</Button_1.Button>
        <Button_1.Button variant="outline" size="sm">Recalculate</Button_1.Button>
      </div>)
    }));
        ...
        <Card_1.Card className="p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex space-x-3">
              <Button_1.Button variant="outline">Refresh All</Button_1.Button>
              <Button_1.Button variant="primary">New Assessment</Button_1.Button>
            </div>
          </div>
...
exports.default = ExpectedCreditLoss;
//# sourceMappingURL=expected-credit-loss.js.map