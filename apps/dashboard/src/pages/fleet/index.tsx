...
const FleetDashboardPage: NextPage = () => {
  ...
  const processFleetMetrics = (vehicles: Vehicle[], drivers: Driver[]): FleetMetrics => {
    ...
    return {
      ...
      maintenanceSchedule,
      fuelConsumption: [ // This would come from actual fuel consumption data
  ...
  const handleAddVehicle = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await fleetService.createVehicle(vehicleFormData);
  ...
  const handleAddDriver = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await fleetService.createDriver(driverFormData);
  ...
  const vehicleColumns = [
    { key: 'vehicleNumber' as keyof Vehicle, header: 'Vehicle #', width: '12%', sortable: true },
    { key: 'registrationNumber' as keyof Vehicle, header: 'Registration', width: '12%', sortable: true },
    { key: 'make' as keyof Vehicle, header: 'Make/Model', width: '15%', 
      render: (value: string, row: Vehicle) => `${value} ${row.model}`
    },
    { key: 'vehicleType' as keyof Vehicle, header: 'Type', width: '12%', sortable: true },
    { key: 'capacity' as keyof Vehicle, header: 'Capacity', width: '12%',
      render: (value: number, row: Vehicle) => `${value} ${row.unit}`
    },
    { key: 'assignedDriverName' as keyof Vehicle, header: 'Driver', width: '15%',
      render: (value: string) => value || 'Unassigned'
    },
    { key: 'status' as keyof Vehicle, header: 'Status', width: '10%',
      render: (value: string) => (
    ...
    { key: 'id' as keyof Vehicle, header: 'Actions', width: '12%',
  ...
  const driverColumns = [
    { key: 'driverNumber' as keyof Driver, header: 'Driver #', width: '12%', sortable: true },
    { key: 'fullName' as keyof Driver, header: 'Full Name', width: '20%', sortable: true },
    { key: 'licenseNumber' as keyof Driver, header: 'License #', width: '15%', sortable: true },
    { key: 'licenseClass' as keyof Driver, header: 'Class', width: '8%', sortable: true },
    { key: 'phone' as keyof Driver, header: 'Phone', width: '15%', sortable: true },
    { key: 'yearsExperience' as keyof Driver, header: 'Experience', width: '10%',
      render: (value: number) => `${value} years`
    },
    { key: 'safetyRating' as keyof Driver, header: 'Safety Rating', width: '10%',
      render: (value: number) => (
        <span className={`font-medium ${
          value >= 4.5 ? 'text-green-400' :
    ...
    { key: 'status' as keyof Driver, header: 'Status', width: '10%',
      render: (value: string) => (
        <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${
          value === 'ACTIVE' ? 'bg-green-500/20 text-green-400 border border-green-500/30' :
    ...
    <FuturisticDashboardLayout>
      <div className="space-y-8">
        ...
        >
          ...
          <div className="flex space-x-4">
            ...
            <Button 
              ...
              size="sm"
              onClick={() => {
                if (activeTab === 'vehicles') setIsAddVehicleModalOpen(true);
                if (activeTab === 'drivers') setIsAddDriverModalOpen(true);
              }}
            >
              <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        >
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                  ...
                  <CardContent>
                    <PieChart 
                      data={metrics.vehicleTypeBreakdown.map(v => ({
                        name: v.type,
                        value: v.count,
                ...
                <Card>
                  <CardHeader title="Fuel Consumption Trend" />
                  <CardContent>
                    <BarChart 
                      data={metrics.fuelConsumption.map(f => ({
                        name: f.month,
                        value: f.consumption
                      }))}
                    />
              ...
              <Card>
                <CardHeader title="Upcoming Maintenance Schedule" />
                <CardContent>
                  <div className="space-y-4">
                    {metrics.maintenanceSchedule.map((item, index) => (
                      <div key={index} className="flex items-center justify-between p-4 rounded-lg bg-dark-800/50">
                        <div className="flex items-center space-x-4">
                          <div className={`w-3 h-3 rounded-full ${
                            item.daysUntilMaintenance <= 3 ? 'bg-red-500' :
          ...
          {activeTab === 'vehicles' && (
            <Card>
              <CardHeader title="Vehicle Fleet" />
              <CardContent>
                <Table
                  data={vehicles}
          ...
          {activeTab === 'maintenance' && (
            <div className="space-y-6">
              <Card>
                <CardHeader title="Maintenance Overview" />
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Select
              ...
              onChange={(value) => setVehicleFormData({ ...vehicleFormData, vehicleType: value })}
              required
            />
            <Input
              label="Make"
              placeholder="e.g., MAN, Volvo, DAF"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Select
              ...
              onChange={(value) => setVehicleFormData({ ...vehicleFormData, unit: value })}
              required
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input
              ...
              type="date"
              value={vehicleFormData.acquisitionDate}
              onChange={(e) => setVehicleFormData({ ...vehicleFormData, acquisitionDate: e.target.value })}
              required
            />
            <Input
              ...
              type="number"
              step="0.01"
              value={vehicleFormData.acquisitionCost}
              onChange={(e) => setVehicleFormData({ ...vehicleFormData, acquisitionCost: e.target.value })}
              required
            />
          </div>
          ...
          <Input
            ...
            type="number"
            step="0.1"
            placeholder="Average fuel efficiency"
            value={vehicleFormData.fuelEfficiency}
            onChange={(e) => setVehicleFormData({ ...vehicleFormData, fuelEfficiency: e.target.value })}
          />
        </FormModal>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Select
              ...
              onChange={(value) => setDriverFormData({ ...driverFormData, licenseClass: value })}
              required
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input
              ...
              type="number"
              placeholder="Years driving experience"
              value={driverFormData.yearsExperience}
              onChange={(e) => setDriverFormData({ ...driverFormData, yearsExperience: e.target.value })}
              required
            />
            <Select
              label="Initial Safety Rating"
              options={[
              ...
              value={driverFormData.safetyRating}
              onChange={(value) => setDriverFormData({ ...driverFormData, safetyRating: value })}
              required
            />
          </div>
        </FormModal>
