...
const DriversPage: NextPage = () => {
  ...
  const generateMockDriverData = (): Driver[] => [
    {
      id: 'd1',
      employeeId: 'EMP001',
      firstName: 'Kwame',
  ...
  const handleSaveDriver = async () => {
    try {
      ...
      if (isEditMode && selectedDriver) {
        await fleetService.updateDriver(selectedDriver.id, driverData);
        toast.success('Driver updated successfully');
      } else {
        await fleetService.createDriver(driverData);
  ...
  const handleAssignVehicle = async (driverId: string, vehicleId: string) => {
    try {
      await fleetService.assignDriver(driverId, vehicleId);
  ...
  const handleUnassignDriver = async (driverId: string) => {
    try {
      await fleetService.unassignDriver(driverId);
  ...
  const getRatingStars = (rating: number) => {
    return Array.from({ length: 5 }, (_, i) => (
      <svg
        key={i}
        className={`w-4 h-4 ${i < Math.floor(rating) ? 'text-yellow-400' : 'text-dark-400'}`}
        fill="currentColor"
        viewBox="0 0 20 20"
      >
  ...
  const availableVehicles = vehicles.filter(vehicle => 
    !drivers.some(driver => driver.assignedVehicle?.id === vehicle.id)
  );
  ...
  return (
    <FuturisticDashboardLayout>
      <div className="space-y-8">
        ...
        {/* Drivers Table */}
        <Card>
          <CardHeader title="Fleet Drivers" />
          <CardContent>
            {loading ? (
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Select
                ...
                onChange={(value) => setDriverForm({ ...driverForm, status: value as any })}
                options={[
                  { value: 'active', label: 'Active' },
                  { value: 'inactive', label: 'Inactive' },
                  { value: 'suspended', label: 'Suspended' },
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <Select
                ...
                onChange={(value) => setDriverForm({ ...driverForm, licenseClass: value })}
                options={[
            ...
            <div>
              ...
              <div className="space-y-2">
                {availableVehicles.length > 0 ? availableVehicles.map(vehicle => (
                  <div key={vehicle.id} className="flex items-center justify-between p-4 bg-dark-700 rounded-lg">
                    <div>
                    ...
                    <Button
                      variant="primary"
                      size="sm"
                      onClick={() => assignmentDriver && handleAssignVehicle(assignmentDriver.id, vehicle.id)}
                    >
                      Assign
                    </Button>
                  </div>
                )) : (
                  <p className="text-dark-400 text-center py-8">No available vehicles to assign</p>
                )}
              </div>
            </div>
