...
const dashboard_stats_1 = require("./dashboard-stats");
      ...
      <div className="space-y-8">
        ...
        <framer_motion_1.motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex justify-between items-center">
          ...
          <div className="flex space-x-4">
            <framer_motion_1.motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="glass rounded-xl px-6 py-3 text-sm font-medium text-white border border-white/10 
                       hover:bg-white/10 transition-all duration-300">
              Export Report
            </framer_motion_1.motion.button>
            <framer_motion_1.motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="bg-gradient-primary text-white rounded-xl px-6 py-3 text-sm font-medium 
        ...
        {/* Key Metrics */}
        <dashboard_stats_1.DashboardStats />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-8">
            ...
            {(user?.role === shared_1.UserRole.SUPER_ADMIN || user?.role === shared_1.UserRole.COMPANY_ADMIN) && (<SystemHealthCard />)}
          </div>
        </div>
      </div>
...
function PricingStatusCard() {
    return (<framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass rounded-3xl border border-white/10 p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        ].map((item, index) => (<div key={item.fuel} className="glass rounded-xl p-4 border border-white/10">
...
function RecentTransactionsCard() {
    return (<framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass rounded-3xl border border-white/10 p-8">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-xl font-semibold text-white">Live Transactions</h3>
        <framer_motion_1.motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 2, repeat: Infinity }} className="px-3 py-1 bg-green-500/20 text-green-400 text-sm font-medium rounded-full border border-green-500/30">
          Live
        </framer_motion_1.motion.div>
      </div>
      
      <div className="space-y-4">
        {[1, 2, 3, 4, 5].map((i) => (<framer_motion_1.motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 * i }} className="flex items-center justify-between p-4 glass rounded-xl border border-white/10 hover:bg-white/5 transition-colors">
...
function UPPFClaimsCard() {
    return (<framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="glass rounded-3xl border border-white/10 p-6">
...
function StationStatusCard() {
    return (<framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="glass rounded-3xl border border-white/10 p-6">
...
function SystemAlertsCard() {
    return (<framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="glass rounded-3xl border border-white/10 p-6">
...
function SystemHealthCard() {
    return (<framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="glass rounded-3xl border border-white/10 p-6">
      ...
      <div className="space-y-3">
        ...
        ].map((service) => (<div key={service.name} className="flex items-center justify-between">
            <span className="text-dark-400 text-sm">{service.name}</span>
            <span className={`text-xs px-2 py-1 rounded-full font-medium ${service.status === 'healthy'
