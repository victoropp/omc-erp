...
const AIInsightsPage: NextPage = () => {
  const [insights, setInsights] = useState<AIInsight[]>([]);
  const [forecastData, setForecastData] = useState<ForecastData | null>(null);
  const [fraudData, setFraudData] = useState<FraudDetection | null>(null);
  const [modelPerformance, setModelPerformance] = useState<MLModelPerformance | null>(null);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<'overview' | 'forecasting' | 'fraud' | 'models'>('overview');
  ...
  const sampleFraudData: FraudDetection = {
    riskScore: 23,
    transactions: [
      {
        id: 'TXN-001234',
  ...
  const sampleModelPerformance: MLModelPerformance = {
    models: [
        ...
        >
                  ...
                  <div className="space-y-4">
                    {insights.map((insight) => (
                      <motion.div
                        ...
                        className="flex items-start space-x-4 p-4 rounded-lg bg-dark-800/50"
                      >
                            ...
                            <div className="mt-3">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {insight.actions.map((action, idx) => (
                                  <span key={idx} className="inline-flex px-2 py-1 text-xs bg-blue-500/20 text-blue-400 rounded border border-blue-500/30">
                                    {action}
                                  </span>
                                ))}
                              </div>
          ...
          {activeTab === 'forecasting' && forecastData && (
            <div className="space-y-6">
              <Card>
                <CardHeader title="Demand Forecasting" />
                <CardContent>
                  <LineChart 
                    data={forecastData.demandForecast.map(d => ({
                      name: d.period,
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                  <CardHeader title="Price Forecasting" />
                  <CardContent>
                    <div className="space-y-4">
                      {forecastData.priceForecast.map((forecast, idx) => (
                        <div key={idx} className="p-4 rounded-lg bg-dark-800/50">
                          ...
                          <div className="mt-2 flex flex-wrap gap-1">
                            {forecast.factors.map((factor, factorIdx) => (
                              <span key={factorIdx} className="inline-flex px-2 py-1 text-xs bg-purple-500/20 text-purple-400 rounded">
                                {factor}
                              </span>
                            ))}
                          </div>
                ...
                <Card>
                  <CardHeader title="Inventory Optimization" />
                  <CardContent>
                    <div className="space-y-4">
                      {forecastData.inventoryOptimization.map((item, idx) => (
                        <div key={idx} className="p-4 rounded-lg bg-dark-800/50">
          ...
          {activeTab === 'fraud' && fraudData && (
            <div className="space-y-6">
                ...
                <Card>
                  <CardContent className="p-6 text-center">
                    ...
                    <p className={`text-3xl font-bold mb-1 ${
                      fraudData.riskScore > 70 ? 'text-red-400' :
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {fraudData.transactions.map((transaction) => (
                      <div key={transaction.id} className="p-4 rounded-lg bg-dark-800/50">
                        <div className="flex items-center justify-between mb-2">
                          <span className="font-medium text-white">{transaction.id}</span>
                          <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${
                            transaction.riskLevel === 'CRITICAL' ? 'bg-red-500/20 text-red-400 border border-red-500/30' :
                        ...
                        <div className="flex flex-wrap gap-1">
                          {transaction.flags.map((flag, idx) => (
                            <span key={idx} className="inline-flex px-2 py-1 text-xs bg-red-500/20 text-red-400 rounded">
                              {flag}
                            </span>
                          ))}
                        </div>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {modelPerformance.models.map((model) => (
                      <div key={model.name} className="p-4 rounded-lg bg-dark-800/50">
                        <div className="flex items-center justify-between mb-2">
