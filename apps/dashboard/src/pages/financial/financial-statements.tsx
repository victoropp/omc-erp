...
interface PLAccount {
  accountCode: string;
  accountName: string;
  accountType: 'REVENUE' | 'COGS' | 'OPERATING_EXPENSE' | 'OTHER_INCOME' | 'OTHER_EXPENSE';
...
interface BSAccount {
  accountCode: string;
  accountName: string;
  accountType: 'CURRENT_ASSET' | 'FIXED_ASSET' | 'CURRENT_LIABILITY' | 'LONG_TERM_LIABILITY' | 'EQUITY';
...
const FinancialStatementsPage: NextPage = () => {
  const [activeTab, setActiveTab] = useState<'profit_loss' | 'balance_sheet' | 'cash_flow' | 'ratios'>('profit_loss');
  const [plAccounts, setPLAccounts] = useState<PLAccount[]>([]);
  const [bsAccounts, setBSAccounts] = useState<BSAccount[]>([]);
  ...
  const handleExportStatement = async (format: 'PDF' | 'EXCEL') => {
    try {
      ...
      //   ...reportFilters
      // });
      toast.success(`${activeTab.replace('_', ' ')} exported as ${format}`);
    } catch (error) {
  ...
  // Sample data
  const samplePLAccounts: PLAccount[] = [
    // Revenue
    {
      accountCode: '4000',
  ...
  const sampleBSAccounts: BSAccount[] = [
    // Current Assets
    {
      accountCode: '1000',
  ...
  const sampleRatios: FinancialRatios = {
    liquidity: {
      currentRatio: 2.8,
      quickRatio: 1.9,
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              ...
              <Select
                ...
                value={reportFilters.periodType}
                onChange={(value) => setReportFilters({ ...reportFilters, periodType: value })}
              />
              <Select
                label="Comparison"
                options={comparisonOptions}
                value={reportFilters.comparison}
                onChange={(value) => setReportFilters({ ...reportFilters, comparison: value })}
              />
              <Input
                label="Currency"
                value={reportFilters.currency}
                onChange={(e) => setReportFilters({ ...reportFilters, currency: e.target.value })}
                disabled
              />
            </div>
          </CardContent>
        ...
        >
          {activeTab === 'profit_loss' && (
            <Card>
              <CardHeader 
                ...
                subtitle={`Period: ${reportFilters.startDate} to ${reportFilters.endDate}`}
              />
              <CardContent>
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    {samplePLAccounts.filter(acc => acc.accountType === 'REVENUE').map((account) => (
                      <div key={account.accountCode} className="grid grid-cols-5 gap-4 py-2 text-sm">
                  ...
                  <div>
                    ...
                    {samplePLAccounts.filter(acc => acc.accountType === 'COGS').map((account) => (
                      <div key={account.accountCode} className="grid grid-cols-5 gap-4 py-2 text-sm">
                        <div className="text-dark-300">{account.accountCode}</div>
                        <div className="text-white">{account.accountName}</div>
                        <div className="text-right text-white">GHS {formatCurrency(account.currentPeriod)}</div>
                  ...
                  <div>
                    ...
                    {samplePLAccounts.filter(acc => acc.accountType === 'OPERATING_EXPENSE').map((account) => (
                      <div key={account.accountCode} className="grid grid-cols-5 gap-4 py-2 text-sm">
                        <div className="text-dark-300">{account.accountCode}</div>
                        <div className="text-white">{account.accountName}</div>
                        <div className="text-right text-white">GHS {formatCurrency(account.currentPeriod)}</div>
          ...
          {activeTab === 'balance_sheet' && (
            <Card>
              <CardHeader 
                title="Balance Sheet" 
                subtitle={`As of ${reportFilters.endDate}`}
              />
              <CardContent>
                <div className="space-y-6">
                  {/* Assets */}
                  <div>
                    ...
                    <div className="mb-4">
                      ...
                      {sampleBSAccounts.filter(acc => acc.accountType === 'CURRENT_ASSET').map((account) => (
                        <div key={account.accountCode} className="grid grid-cols-4 gap-4 py-2 text-sm">
                          <div className="text-white">{account.accountName}</div>
                    ...
                    <div>
                      ...
                      {sampleBSAccounts.filter(acc => acc.accountType === 'FIXED_ASSET').map((account) => (
                        <div key={account.accountCode} className="grid grid-cols-4 gap-4 py-2 text-sm">
                          <div className="text-white">{account.accountName}</div>
                          <div className="text-right text-white">GHS {formatCurrency(account.currentBalance)}</div>
                          <div className="text-right text-dark-400">GHS {formatCurrency(account.previousBalance)}</div>
                  ...
                  <div>
                    ...
                    <div className="mb-4">
                      ...
                      {sampleBSAccounts.filter(acc => acc.accountType === 'CURRENT_LIABILITY').map((account) => (
                        <div key={account.accountCode} className="grid grid-cols-4 gap-4 py-2 text-sm">
                          <div className="text-white">{account.accountName}</div>
                          <div className="text-right text-white">GHS {formatCurrency(account.currentBalance)}</div>
                          <div className="text-right text-dark-400">GHS {formatCurrency(account.previousBalance)}</div>
                    ...
                    <div className="mb-4">
                      ...
                      {sampleBSAccounts.filter(acc => acc.accountType === 'EQUITY').map((account) => (
                        <div key={account.accountCode} className="grid grid-cols-4 gap-4 py-2 text-sm">
                          <div className="text-white">{account.accountName}</div>
                          <div className="text-right text-white">GHS {formatCurrency(account.currentBalance)}</div>
                          <div className="text-right text-dark-400">GHS {formatCurrency(account.previousBalance)}</div>
              ...
              <CardContent>
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    {sampleCashFlowItems.filter(item => item.category === 'OPERATING').map((item, index) => (
                  ...
                  <div>
                    ...
                    {sampleCashFlowItems.filter(item => item.category === 'INVESTING').map((item, index) => (
                      <div key={index} className="grid grid-cols-3 gap-4 py-2 text-sm">
                        <div className="text-white">{item.description}</div>
                        <div className={`text-right ${item.currentPeriod >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                          GHS {formatCurrency(item.currentPeriod)}
                  ...
                  <div>
                    ...
                    {sampleCashFlowItems.filter(item => item.category === 'FINANCING').map((item, index) => (
                      <div key={index} className="grid grid-cols-3 gap-4 py-2 text-sm">
                        <div className="text-white">{item.description}</div>
                        <div className={`text-right ${item.currentPeriod >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                          GHS {formatCurrency(item.currentPeriod)}
          ...
          {activeTab === 'ratios' && ratios && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card>
                <CardHeader title="Liquidity Ratios" />
                <CardContent>
              ...
              <Card>
                <CardHeader title="Profitability Ratios" />
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center p-3 bg-dark-800/30 rounded-lg">
              ...
              <Card>
                <CardHeader title="Efficiency Ratios" />
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center p-3 bg-dark-800/30 rounded-lg">
              ...
              <Card>
                <CardHeader title="Leverage Ratios" />
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center p-3 bg-dark-800/30 rounded-lg">
