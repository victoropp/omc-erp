...
const DealerCreditManagement = () => {
  ...
  const loadCreditData = async () => {
    ...
    try {
      ...
      toast.error('Failed to load credit assessment data');
      setLoading(false);
    }
  };
  ...
  const handleReassessCredit = async (assessmentId: string) => {
    try {
      // API call to reassess credit
      toast.success('Credit reassessment initiated');
      loadCreditData(); // Reload data
    } catch (error) {
      toast.error('Failed to initiate credit reassessment');
    }
  };
  ...
  if (loading) {
    return (
      <FuturisticDashboardLayout title="Credit Assessment" subtitle="Loading...">
  ...
  return (
    <FuturisticDashboardLayout 
      title="Credit Assessment & Management" 
      subtitle="Comprehensive dealer credit evaluation and loan management system"
    >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          >
            <Card className="p-6">
              ...
              <BarChart data={creditScoreData} height={250} />
            </Card>
          </motion.div>
          ...
          >
            <Card className="p-6">
              ...
              <LineChart data={utilizationTrendData} height={250} />
            </Card>
          </motion.div>
        </div>
                    ...
                    >
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center">
                          ...
                          <div className="w-16 h-2 bg-gray-200 rounded-full ml-2 overflow-hidden">
                            <div 
                              className={`h-full ${assessment.utilization > 90 ? 'bg-red-500' : assessment.utilization > 75 ? 'bg-yellow-500' : 'bg-green-500'}`}
                              style={{ width: `${Math.min(assessment.utilization, 100)}%` }}
                            />
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="py-3 px-4">
                        <div className="flex space-x-2">
                          ...
                          <Button
                            size="sm"
                            onClick={() => handleReassessCredit(assessment.id)}
                          >
                            Reassess
                          </Button>
                        </div>
        ...
        >
          <Card className="p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Button onClick={() => setShowLoanModal(true)}>Process New Loan</Button>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
        ...
        >
          <Card className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <Button className="flex flex-col items-center p-6 h-auto" onClick={() => setShowAssessmentModal(true)}>
      ...
      <Modal
        ...
        title={selectedAssessment ? 'Credit Assessment Details' : 'New Credit Assessment'}
        size="large"
      >
        <div className="space-y-4">
          {selectedAssessment ? (
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Badge variant={selectedAssessment.paymentHistory === 'EXCELLENT' ? 'success' : selectedAssessment.paymentHistory === 'GOOD' ? 'primary' : 'warning'}>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              />
              <Input label="Assessment Date" type="date" />
              <Input label="Monthly Turnover" type="number" placeholder="1000000" />
              <Input label="Collateral Value" type="number" placeholder="500000" />
              <Input label="Guarantees" type="number" placeholder="100000" />
              <Select
          ...
          <div className="flex justify-end space-x-3 mt-6">
            ...
            {!selectedAssessment && (
              <Button onClick={() => {
                toast.success('Credit assessment created successfully');
                setShowAssessmentModal(false);
                loadCreditData();
              }}>
                Create Assessment
      ...
      <Modal
        ...
        title="Process New Loan Application"
        size="large"
      >
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            />
            <Input label="Requested Amount" type="number" placeholder="150000" />
            <Input label="Purpose" placeholder="Equipment upgrade" />
            <Input label="Term (months)" type="number" placeholder="24" />
            <Input label="Interest Rate %" type="number" step="0.1" placeholder="8.5" />
            <Input label="Application Date" type="date" />
          </div>
...
export default DealerCreditManagement;