...
const DealerReports = () => {
    ...
    const [reportType, setReportType] = (0, react_1.useState)('ALL');
    const [selectedDealer, setSelectedDealer] = (0, react_1.useState)('ALL');
    ...
    const [customReportBuilder, setCustomReportBuilder] = (0, react_1.useState)({});
    const [showCustomBuilder, setShowCustomBuilder] = (0, react_1.useState)(false);
    ...
    const loadReportsData = async () => {
        ...
        try {
            ...
            react_hot_toast_1.toast.error('Failed to load reports data');
            setLoading(false);
        }
    };
    ...
    const generateReport = async (type, dealerId) => {
        try {
    ...
    const exportReport = async (report, format) => {
        try {
            react_hot_toast_1.toast.loading(`Exporting as ${format}...`);
            ...
            react_hot_toast_1.toast.dismiss();
            react_hot_toast_1.toast.success(`Report exported as ${format}`);
        }
        catch (error) {
            react_hot_toast_1.toast.dismiss();
            react_hot_toast_1.toast.error('Failed to export report');
        }
    };
    const scheduleReport = async (config) => {
        try {
            react_hot_toast_1.toast.loading('Scheduling report...');
    ...
    if (loading) {
        return (<FuturisticDashboardLayout_1.FuturisticDashboardLayout title="Dealer Reports" subtitle="Loading...">
        <div className="flex items-center justify-center h-64">
          <div className="animate-spin rounded-full h-32 w-32 border-b-2 border-blue-600"></div>
        </div>
      </FuturisticDashboardLayout_1.FuturisticDashboardLayout>);
    }
    return (<FuturisticDashboardLayout_1.FuturisticDashboardLayout title="Comprehensive Dealer Reports" subtitle="Advanced reporting and analytics for dealer performance optimization">
      <div className="space-y-6">
        {/* Control Panel */}
        <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <Card_1.Card className="p-6 bg-gradient-to-r from-blue-600/10 to-purple-600/10 backdrop-blur-sm border border-blue-500/20">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              <Select_1.Select value={reportType} onChange={(e) => setReportType(e.target.value)} options={[
            ...
            { value: 'SALES_REVENUE', label: 'Sales & Revenue' }
              ...
              <Select_1.Select value={selectedDealer} onChange={(e) => setSelectedDealer(e.target.value)} options={[
            ...
            { value: 'DLR-003', label: 'Star Oil Ltd' }
              ...
              <Select_1.Select value={selectedRegion} onChange={(e) => setSelectedRegion(e.target.value)} options={[
            ...
            { value: 'Northern', label: 'Northern' }
              ...
              <Input_1.Input type="date" value={dateRange.start} onChange={(e) => setDateRange(prev => ({ ...prev, start: e.target.value }))} className="bg-white/10 backdrop-blur-sm border-white/20"/>
              
              <Input_1.Input type="date" value={dateRange.end} onChange={(e) => setDateRange(prev => ({ ...prev, end: e.target.value }))} className="bg-white/10 backdrop-blur-sm border-white/20"/>
            </div>
            
            <div className="flex flex-wrap gap-3 mt-4">
              <Button_1.Button onClick={() => generateReport('PERFORMANCE')} className="bg-gradient-to-r from-blue-600 to-blue-700">
                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
                Generate Report
              </Button_1.Button>
              <Button_1.Button variant="outline" onClick={() => setShowCustomBuilder(true)}>
                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 100-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 100-4m0 4v2m0-6V4"/>
                </svg>
                Custom Report Builder
              </Button_1.Button>
              <Button_1.Button variant="outline" onClick={loadReportsData}>
                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            <Card_1.Card className="p-6 bg-gradient-to-br from-blue-600/20 to-blue-800/20 backdrop-blur-sm border border-blue-500/30">
          ...
          <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
            <Card_1.Card className="p-6 bg-gradient-to-br from-green-600/20 to-green-800/20 backdrop-blur-sm border border-green-500/30">
              <div className="flex items-center justify-between">
                <div>
          ...
          <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
            <Card_1.Card className="p-6 bg-gradient-to-br from-purple-600/20 to-purple-800/20 backdrop-blur-sm border border-purple-500/30">
              <div className="flex items-center justify-between">
                <div>
          ...
          <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
            <Card_1.Card className="p-6 bg-gradient-to-br from-orange-600/20 to-orange-800/20 backdrop-blur-sm border border-orange-500/30">
              <div className="flex items-center justify-between">
                <div>
        ...
        <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
          <Card_1.Card className="p-6">
        ...
        ].map((tab) => (<button key={tab.id} className={`pb-2 px-1 font-medium transition-colors ${activeTab === tab.id
                ? 'border-b-2 border-blue-600 text-blue-600'
                : 'text-gray-600 hover:text-blue-600'}`} onClick={() => setActiveTab(tab.id)}>
            ...
            <framer_motion_1.AnimatePresence mode="wait">
              {activeTab === 'overview' && (<framer_motion_1.motion.div key="overview" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-6">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div>
                      <h4 className="text-lg font-semibold mb-4">Performance Metrics Comparison</h4>
                      <charts_1.BarChart data={performanceMetricsData} height={300}/>
                    </div>
                    <div>
                      <h4 className="text-lg font-semibold mb-4">Revenue Distribution</h4>
                      <charts_1.PieChart data={revenueComparisonData} height={300}/>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {filteredReports.slice(0, 5).map((report, index) => (<framer_motion_1.motion.div key={report.dealerId + report.reportType} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                          ...
                          <div className="flex items-center space-x-3">
                            <ui_1.Badge variant={report.status === 'READY' ? 'success' : 'warning'}>
                              {report.status}
                            </ui_1.Badge>
                            <div className="flex space-x-1">
                              <Button_1.Button size="sm" variant="outline" onClick={() => exportReport(report, 'PDF')}>
                                PDF
                              </Button_1.Button>
                              <Button_1.Button size="sm" variant="outline" onClick={() => exportReport(report, 'EXCEL')}>
                                Excel
                              </Button_1.Button>
                            </div>
                          </div>
              ...
              {activeTab === 'performance' && (<framer_motion_1.motion.div key="performance" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-6">
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    {performanceData.map((dealer, index) => (<framer_motion_1.motion.div key={dealer.dealerId} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="p-6 bg-gradient-to-br from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 rounded-lg border">
                        <div className="flex items-center justify-between mb-4">
                          <h5 className="font-semibold">{dealer.dealerName}</h5>
                          <ui_1.Badge variant="success">
                        ...
                        <div className="mt-4 flex space-x-2">
                          <Button_1.Button size="sm" className="flex-1" onClick={() => generateReport('PERFORMANCE', dealer.dealerId)}>
                            Generate Report
                          </Button_1.Button>
                          <Button_1.Button size="sm" variant="outline" onClick={() => setSelectedReport(reports.find(r => r.dealerId === dealer.dealerId && r.reportType === 'PERFORMANCE') || null)}>
                            View Details
                          </Button_1.Button>
                        </div>
                      </framer_motion_1.motion.div>))}
              ...
              {activeTab === 'settlement' && (<framer_motion_1.motion.div key="settlement" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-6">
                  {settlementData.map((settlement, index) => (<framer_motion_1.motion.div key={settlement.dealerId} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="p-6 bg-gradient-to-r from-green-50 to-blue-50 dark:from-green-900/20 dark:to-blue-900/20 rounded-lg border">
                      <div className="flex items-center justify-between mb-6">
                        <h5 className="text-xl font-semibold">{settlement.dealerName} - Settlement Report</h5>
                        <div className="flex space-x-2">
                          <ui_1.Badge variant={settlement.overdueAmount > 0 ? 'warning' : 'success'}>
                            {settlement.overdueAmount > 0 ? 'Has Overdue' : 'Current'}
                          </ui_1.Badge>
                          <Button_1.Button size="sm" onClick={() => exportReport(reports.find(r => r.dealerId === settlement.dealerId && r.reportType === 'SETTLEMENT'), 'PDF')}>
                            Export PDF
                          </Button_1.Button>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div>
                          ...
                          <div className="space-y-2">
                            {Object.entries(settlement.settlementBreakdown).map(([key, value]) => (<div key={key} className="flex justify-between items-center p-2 bg-white dark:bg-gray-800 rounded">
                      ...
                      <div className="mt-6">
                        ...
                        <div className="overflow-x-auto">
                          <table className="w-full text-sm">
                            ...
                            <tbody>
                              {settlement.paymentHistory.map((payment, idx) => (<tr key={idx} className="border-b">
                                  ...
                                  <td className="py-2">
                                    <ui_1.Badge variant="success">{payment.status}</ui_1.Badge>
                                  </td>
                                </tr>))}
                            </tbody>
                          </table>
              ...
              {activeTab === 'compliance' && (<framer_motion_1.motion.div key="compliance" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-6">
                  {complianceData.map((compliance, index) => (<framer_motion_1.motion.div key={compliance.dealerId} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="p-6 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-lg border">
                      <div className="flex items-center justify-between mb-6">
                        <h5 className="text-xl font-semibold">{compliance.dealerName} - Compliance Report</h5>
                        <div className="flex items-center space-x-3">
                          <ui_1.Badge variant={compliance.overallScore >= 90 ? 'success' : compliance.overallScore >= 80 ? 'warning' : 'danger'}>
                            Overall Score: {compliance.overallScore.toFixed(1)}%
                          </ui_1.Badge>
                          <Button_1.Button size="sm" onClick={() => exportReport(reports.find(r => r.dealerId === compliance.dealerId && r.reportType === 'COMPLIANCE'), 'PDF')}>
                            Export Report
                          </Button_1.Button>
                        </div>
                      </div>
                    ...
                    { label: 'Safety Compliance', value: compliance.safetyCompliance, color: 'bg-red-500' }
                        ...
                        <div>
                          ...
                          <div className="space-y-3">
                            {compliance.violations.length > 0 ? compliance.violations.map((violation, idx) => (<div key={idx} className="p-3 bg-white dark:bg-gray-800 rounded-lg border">
                                <div className="flex items-center justify-between mb-2">
                                  <span className="font-medium text-sm">{violation.type}</span>
                                  <ui_1.Badge variant={violation.severity === 'CRITICAL' ? 'danger' :
                        violation.severity === 'HIGH' ? 'warning' : 'outline'}>
                                ...
                                <div className="flex justify-between items-center">
                                  ...
                                  <ui_1.Badge variant={violation.status === 'RESOLVED' ? 'success' : 'warning'}>
                                    {violation.status}
                                  </ui_1.Badge>
                                </div>
                                {violation.penalty && (<p className="text-xs text-red-600 mt-1">Penalty: ₵{violation.penalty}</p>)}
                              </div>)) : (<p className="text-sm text-green-600">No violations found</p>)}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div>
                          ...
                          <div className="space-y-2">
                            {compliance.certifications.map((cert, idx) => (<div key={idx} className="flex items-center justify-between p-2 bg-white dark:bg-gray-800 rounded">
                                <span className="text-sm">{cert.name}</span>
                                <div className="flex items-center space-x-2">
                                  <ui_1.Badge variant={cert.status === 'VALID' ? 'success' :
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            {compliance.auditHistory.map((audit, idx) => (<div key={idx} className="flex items-center justify-between p-2 bg-white dark:bg-gray-800 rounded">
                                <div>
              ...
              {activeTab === 'schedule' && (<framer_motion_1.motion.div key="schedule" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-6">
                  <div className="flex items-center justify-between">
                ...
                scheduleReport(config);
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {scheduledReports.map((scheduled, index) => (<framer_motion_1.motion.div key={scheduled.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="p-6 bg-gradient-to-r from-indigo-50 to-blue-50 dark:from-indigo-900/20 dark:to-blue-900/20 rounded-lg border">
                        <div className="flex items-center justify-between mb-4">
                          <h5 className="font-semibold">{scheduled.name}</h5>
                          <ui_1.Badge variant={scheduled.status === 'ACTIVE' ? 'success' : 'warning'}>
                            {scheduled.status}
                          </ui_1.Badge>
                        </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div>
                            ...
                            <div className="mt-1 flex flex-wrap gap-1">
                              {scheduled.recipients.map((email, idx) => (<ui_1.Badge key={idx} variant="outline" className="text-xs">
                                  {email}
                                </ui_1.Badge>))}
                            </div>
                          </div>
        ...
        <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }}>
          <Card_1.Card className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <Button_1.Button className="flex flex-col items-center p-6 h-auto bg-gradient-to-r from-blue-600 to-blue-700" onClick={() => generateReport('PERFORMANCE')}>
                <svg className="w-8 h-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
                <span>Bulk Export</span>
              </Button_1.Button>
              <Button_1.Button variant="outline" className="flex flex-col items-center p-6 h-auto" onClick={() => setShowCustomBuilder(true)}>
                <svg className="w-8 h-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 100-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 100-4m0 4v2m0-6V4"/>
                </svg>
