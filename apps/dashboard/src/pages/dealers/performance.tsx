...
const DealerPerformance: NextPage = () => {
  const [performanceData] = useState<DealerPerformance[]>([
    {
      id: '1',
  ...
  const tableData = filteredData.map(dealer => ({
    ...
    performance: (
      <div className="text-sm">
        ...
        <div className={`${
          dealer.salesVolume >= dealer.targetVolume ? 'text-green-400' : 'text-yellow-400'
        }`}>
    ...
    overallRating: (
      <span className={`px-2 py-1 rounded-full text-xs font-medium border ${getRatingColor(dealer.overallRating)}`}>
