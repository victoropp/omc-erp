...
import { complianceService, regulatoryService } from '@/services/api';
...
interface ComplianceCheck {
  ...
  dealerName: string;
  location: string;
  checkType: 'MARGIN' | 'QUALITY' | 'SAFETY' | 'ENVIRONMENTAL' | 'FINANCIAL' | 'OPERATIONAL';
  checkDate: string;
  inspector: string;
  status: 'COMPLIANT' | 'NON_COMPLIANT' | 'PENDING' | 'NEEDS_REVIEW';
  score: number;
  maxScore: number;
  findings: string[];
...
interface RegulatoryRequirement {
  ...
  title: string;
  description: string;
  regulator: 'NPA' | 'EPA' | 'GRA' | 'BOG' | 'FIRE_SERVICE' | 'WEIGHTS_MEASURES';
  category: 'LICENSING' | 'REPORTING' | 'SAFETY' | 'ENVIRONMENTAL' | 'FINANCIAL' | 'OPERATIONAL';
  frequency: 'DAILY' | 'WEEKLY' | 'MONTHLY' | 'QUARTERLY' | 'ANNUALLY';
  dueDate: string;
  status: 'COMPLIANT' | 'OVERDUE' | 'UPCOMING' | 'NOT_APPLICABLE';
  lastUpdated: string;
  assignedTo: string;
}
...
interface ComplianceAlert {
  ...
  dealerId: string;
  dealerName: string;
  alertType: 'MARGIN_BREACH' | 'QUALITY_ISSUE' | 'SAFETY_VIOLATION' | 'ENVIRONMENTAL_ISSUE' | 'EXPIRED_LICENSE';
...
const DealerCompliance = () => {
  ...
  const handleScheduleCheck = async () => {
    try {
      ...
      toast.success('Compliance check scheduled successfully');
  ...
  if (loading) {
    return (
      <FuturisticDashboardLayout title="Dealer Compliance" subtitle="Loading...">
  ...
  return (
    <FuturisticDashboardLayout 
      ...
      subtitle="Comprehensive dealer compliance tracking and regulatory management system"
    >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          >
            <Card className="p-6">
              ...
              <PieChart data={complianceStatusData} height={250} />
            </Card>
          </motion.div>
          ...
          >
            <Card className="p-6">
              ...
              <BarChart data={complianceTypeData} height={250} />
            </Card>
          </motion.div>
          ...
          >
            <Card className="p-6">
              ...
              <LineChart data={complianceTrendData} height={250} />
            </Card>
          </motion.div>
        </div>
        ...
        >
          <Card className="p-6">
            ...
            <div className="space-y-4">
              {complianceAlerts.filter(a => a.status === 'ACTIVE' || a.status === 'ACKNOWLEDGED').map((alert, index) => (
                <motion.div
                  key={alert.id}
                  initial={{ opacity: 0, x: -20 }}
                  animate={{ opacity: 1, x: 0 }}
        ...
        >
          <Card className="p-6">
            <div className="flex space-x-4 border-b mb-4">
              <button
                className={`pb-2 px-1 font-medium ${selectedTab === 'checks' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-600'}`}
                onClick={() => setSelectedTab('checks')}
              >
                Compliance Checks
              </button>
              <button
                className={`pb-2 px-1 font-medium ${selectedTab === 'requirements' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-600'}`}
                onClick={() => setSelectedTab('requirements')}
              >
                Regulatory Requirements
              </button>
              <button
                className={`pb-2 px-1 font-medium ${selectedTab === 'alerts' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-600'}`}
                    ...
                    <tbody>
                      {filteredChecks.map((check, index) => (
                        ...
                        >
                          ...
                          <td className="py-3 px-4">
                            <Badge variant="outline">{check.checkType.replace('_', ' ')}</Badge>
                          </td>
                          <td className="py-3 px-4">
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {regulatoryRequirements.map((requirement, index) => (
                        <tr key={requirement.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                          <td className="py-3 px-4">
                            <div>
                          ...
                          <td className="py-3 px-4">
                            <Badge variant="outline">{requirement.regulator}</Badge>
                          </td>
                  ...
                  {complianceAlerts.map((alert, index) => (
                    <div
                      ...
                      className={`p-4 rounded-lg border ${
                        alert.status === 'RESOLVED' ? 'border-gray-200 bg-gray-50 dark:bg-gray-800' :
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-600">
                            ...
                            {alert.resolvedDate && (
                              <span> | Resolved: {new Date(alert.resolvedDate).toLocaleString()} by {alert.resolvedBy}</span>
                            )}
                          </p>
                        </div>
                        <Badge variant={alert.alertType.includes('CRITICAL') || alert.alertType.includes('EXPIRED') ? 'danger' : 'warning'}>
        ...
        >
          <Card className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <Button className="flex flex-col items-center p-6 h-auto" onClick={() => setShowCheckModal(true)}>
      ...
      <Modal
        ...
        title={selectedCheck ? 'Compliance Check Details' : 'Schedule Compliance Check'}
        size="large"
      >
        <div className="space-y-4">
          {selectedCheck ? (
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Badge variant="outline">{selectedCheck.checkType.replace('_', ' ')}</Badge>
              </div>
              <div>
...
export default DealerCompliance;