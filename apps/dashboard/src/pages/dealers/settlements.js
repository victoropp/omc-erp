...
const DealerSettlements = () => {
    ...
    const [settlementBatches, setSettlementBatches] = (0, react_1.useState)([]);
    const [paymentInstructions, setPaymentInstructions] = (0, react_1.useState)([]);
    ...
    const [showBatchModal, setShowBatchModal] = (0, react_1.useState)(false);
    ...
    const [filterPeriod, setFilterPeriod] = (0, react_1.useState)('ALL');
    const [selectedTab, setSelectedTab] = (0, react_1.useState)('settlements');
    (0, react_1.useEffect)(() => {
        loadSettlementData();
    }, []);
    const loadSettlementData = async () => {
        ...
        try {
            ...
            react_hot_toast_1.toast.error('Failed to load settlement data');
            setLoading(false);
        }
    };
    ...
    const handleProcessSettlement = async (settlementId) => {
        try {
            ...
            react_hot_toast_1.toast.success('Settlement processed successfully');
            loadSettlementData();
        }
        catch (error) {
            react_hot_toast_1.toast.error('Failed to process settlement');
        }
    };
    const handleApprovePayment = async (paymentId) => {
        try {
            // API call to approve payment
            react_hot_toast_1.toast.success('Payment approved and sent to bank');
            loadSettlementData();
        }
        catch (error) {
            react_hot_toast_1.toast.error('Failed to approve payment');
        }
    };
    const handleCreateBatch = async () => {
        try {
            // API call to create settlement batch
            react_hot_toast_1.toast.success('Settlement batch created successfully');
            setShowBatchModal(false);
            loadSettlementData();
        }
        catch (error) {
            react_hot_toast_1.toast.error('Failed to create settlement batch');
        }
    };
    ...
    if (loading) {
        return (<FuturisticDashboardLayout_1.FuturisticDashboardLayout title="Dealer Settlements" subtitle="Loading...">
        <div className="flex items-center justify-center h-64">
          <div className="animate-spin rounded-full h-32 w-32 border-b-2 border-blue-600"></div>
        </div>
      </FuturisticDashboardLayout_1.FuturisticDashboardLayout>);
    }
    return (<FuturisticDashboardLayout_1.FuturisticDashboardLayout title="Dealer Settlement Processing" subtitle="Automated dealer settlement calculations and payment processing system">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <framer_motion_1.motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }}>
            <Card_1.Card className="p-6">
              ...
              <charts_1.PieChart data={settlementStatusData} height={250}/>
            </Card_1.Card>
          </framer_motion_1.motion.div>
          ...
          <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
            <Card_1.Card className="p-6">
              ...
              <charts_1.LineChart data={settlementTrendData} height={250}/>
            </Card_1.Card>
          </framer_motion_1.motion.div>
          ...
          <framer_motion_1.motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.7 }}>
            <Card_1.Card className="p-6">
              ...
              <charts_1.BarChart data={fuelTypeBreakdownData} height={250}/>
            </Card_1.Card>
          </framer_motion_1.motion.div>
        </div>
        ...
        <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }}>
          <Card_1.Card className="p-6">
            <div className="flex space-x-4 border-b mb-4">
              <button className={`pb-2 px-1 font-medium ${selectedTab === 'settlements' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-600'}`} onClick={() => setSelectedTab('settlements')}>
                Settlements
              </button>
              <button className={`pb-2 px-1 font-medium ${selectedTab === 'batches' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-600'}`} onClick={() => setSelectedTab('batches')}>
                Batches
              </button>
              <button className={`pb-2 px-1 font-medium ${selectedTab === 'payments' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-600'}`} onClick={() => setSelectedTab('payments')}>
                Payments
              </button>
            </div>

            {selectedTab === 'settlements' && (<div>
                {/* Filters and Search */}
                ...
                { value: 'DISPUTED', label: 'Disputed' }
                  ...
                  <Select_1.Select value={filterPeriod} onChange={(e) => setFilterPeriod(e.target.value)} options={[
                { value: 'ALL', label: 'All Periods' },
                { value: 'THIS_MONTH', label: 'This Month' },
                { value: 'LAST_MONTH', label: 'Last Month' },
                { value: 'THIS_QUARTER', label: 'This Quarter' }
            ]}/>
                  <Button_1.Button onClick={() => setShowSettlementModal(true)}>
                    New Settlement
                  </Button_1.Button>
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {filteredSettlements.map((settlement, index) => (<framer_motion_1.motion.tr key={settlement.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9 + index * 0.1 }} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                    ...
                    setShowSettlementModal(true);
                              ...
                              {settlement.status === 'PENDING' && (<Button_1.Button size="sm" onClick={() => handleProcessSettlement(settlement.id)}>
                                  Process
                                </Button_1.Button>)}
                            </div>
                          </td>
            ...
            {selectedTab === 'batches' && (<div>
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-lg font-semibold">Settlement Batches</h3>
                  <Button_1.Button onClick={() => setShowBatchModal(true)}>Create New Batch</Button_1.Button>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                    ...
                    <tbody>
                      {settlementBatches.map((batch, index) => (<tr key={batch.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                          ...
                          <td className="py-3 px-4">
                            <div className="flex items-center">
                              ...
                              <div className="w-16 h-2 bg-gray-200 rounded-full ml-2 overflow-hidden">
                                <div className="h-full bg-blue-500" style={{ width: `${(batch.processedCount / batch.totalDealers) * 100}%` }}/>
                              </div>
                            </div>
                          </td>
                          <td className="py-3 px-4">
                          ...
                          <td className="py-3 px-4">
                            <Button_1.Button size="sm" variant="outline">View Details</Button_1.Button>
                          </td>
                        </tr>))}
                    </tbody>
                  </table>
            ...
            {selectedTab === 'payments' && (<div>
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-lg font-semibold">Payment Instructions</h3>
                  <Button_1.Button onClick={() => setShowPaymentModal(true)}>Generate Payments</Button_1.Button>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                    ...
                    <tbody>
                      {paymentInstructions.map((payment, index) => (<tr key={payment.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                          ...
                          <td className="py-3 px-4">
                            {payment.status === 'PENDING' && (<Button_1.Button size="sm" onClick={() => handleApprovePayment(payment.id)}>
                                Approve
                              </Button_1.Button>)}
                            {payment.status === 'CONFIRMED' && (<Button_1.Button size="sm" variant="outline">View Receipt</Button_1.Button>)}
                          </td>
                        </tr>))}
                    </tbody>
                  </table>
        ...
        <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.0 }}>
          <Card_1.Card className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <Button_1.Button className="flex flex-col items-center p-6 h-auto" onClick={() => setShowBatchModal(true)}>
      ...
      <Modal_1.Modal isOpen={showSettlementModal} onClose={() => {
            setShowSettlementModal(false);
            setSelectedSettlement(null);
        }} title={selectedSettlement ? 'Settlement Details' : 'New Settlement'} size="large">
        <div className="space-y-4">
          {selectedSettlement ? (<div className="grid grid-cols-2 gap-4">
              <div>
              ...
              <Input_1.Input label="Period Start" type="date"/>
              <Input_1.Input label="Period End" type="date"/>
              <Input_1.Input label="Gross Sales" type="number" placeholder="1000000"/>
              <Input_1.Input label="Commission Rate %" type="number" step="0.1" placeholder="5.0"/>
              <Input_1.Input label="Discounts" type="number" placeholder="5000"/>
            </div>)}
          ...
          <div className="flex justify-end space-x-3 mt-6">
            ...
            {!selectedSettlement && (<Button_1.Button onClick={() => {
                react_hot_toast_1.toast.success('Settlement created successfully');
                setShowSettlementModal(false);
                loadSettlementData();
            }}>
                Create Settlement
      ...
      <Modal_1.Modal isOpen={showBatchModal} onClose={() => setShowBatchModal(false)} title="Create Settlement Batch" size="large">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Input_1.Input label="Batch Name" placeholder="January 2025 Bi-Weekly Settlement"/>
            <Select_1.Select label="Settlement Period" options={[
            ...
            <Input_1.Input label="Period Start" type="date"/>
            <Input_1.Input label="Period End" type="date"/>
            <Select_1.Select label="Include Dealers" options={[
            { value: 'all_active', label: 'All Active Dealers' },
            { value: 'specific', label: 'Specific Dealers' },
            { value: 'region', label: 'By Region' }
        ]}/>
            <Select_1.Select label="Commission Type" options={[
          ...
          <div className="flex justify-end space-x-3 mt-6">
            <Button_1.Button variant="outline" onClick={() => setShowBatchModal(false)}>
              Cancel
            </Button_1.Button>
            <Button_1.Button onClick={handleCreateBatch}>
              Create Batch
            </Button_1.Button>
          </div>
        </div>
      ...
      <Modal_1.Modal isOpen={showPaymentModal} onClose={() => setShowPaymentModal(false)} title="Generate Payment Instructions" size="large">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Select_1.Select label="Settlement Batch" options={[
            { value: 'BATCH-001', label: 'January 2025 Bi-Weekly Settlement' },
            { value: 'BATCH-002', label: 'December 2024 Monthly Settlement' }
        ]}/>
            <Select_1.Select label="Payment Method" options={[
            { value: 'bank_transfer', label: 'Bank Transfer' },
            { value: 'cheque', label: 'Cheque' },
            { value: 'mobile_money', label: 'Mobile Money' }
        ]}/>
            <Input_1.Input label="Payment Date" type="date"/>
            <Input_1.Input label="Reference Prefix" placeholder="SETTLE-2025-"/>
          </div>
          ...
          <div className="flex justify-end space-x-3 mt-6">
            <Button_1.Button variant="outline" onClick={() => setShowPaymentModal(false)}>
              Cancel
            </Button_1.Button>
            <Button_1.Button onClick={() => {
            react_hot_toast_1.toast.success('Payment instructions generated successfully');
...
exports.default = DealerSettlements;
