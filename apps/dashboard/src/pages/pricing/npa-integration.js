...
const NPAIntegrationDashboard = () => {
    ...
    const loadNPAData = async () => {
        try {
            ...
            react_hot_toast_1.toast.success('NPA data loaded successfully');
        }
        catch (error) {
            react_hot_toast_1.toast.error('Failed to load NPA data');
    ...
    const syncWithNPA = async () => {
        try {
            react_hot_toast_1.toast.loading('Synchronizing with NPA systems...');
            // Simulate NPA sync
            await new Promise(resolve => setTimeout(resolve, 3000));
            react_hot_toast_1.toast.success('NPA synchronization completed');
            await loadNPAData();
        }
        catch (error) {
            react_hot_toast_1.toast.error('NPA synchronization failed');
        }
    };
    const submitUPPFClaim = async () => {
        try {
            react_hot_toast_1.toast.loading('Submitting UPPF claim to NPA...');
            // Simulate UPPF submission
            await new Promise(resolve => setTimeout(resolve, 2500));
            react_hot_toast_1.toast.success('UPPF claim submitted successfully');
            await loadNPAData();
        }
        catch (error) {
            react_hot_toast_1.toast.error('UPPF submission failed');
        }
    };
    const downloadComplianceReport = async () => {
        try {
            react_hot_toast_1.toast.loading('Generating compliance report...');
            // Simulate report generation
            await new Promise(resolve => setTimeout(resolve, 2000));
            react_hot_toast_1.toast.success('Compliance report downloaded');
        }
        catch (error) {
      ...
      <div className="space-y-6">
        ...
        <framer_motion_1.motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex justify-between items-center">
          ...
          <div className="flex space-x-4">
            <button onClick={syncWithNPA} className="glass px-4 py-2 rounded-lg border border-white/20 bg-white/10 text-white hover:bg-white/20 transition-all">
              Sync with NPA
            </button>
            
            <button onClick={submitUPPFClaim} className="glass px-4 py-2 rounded-lg border border-white/20 bg-white/10 text-white hover:bg-white/20 transition-all">
              Submit UPPF
            </button>
            
            <button onClick={downloadComplianceReport} className="glass px-4 py-2 rounded-lg border border-white/20 bg-white/10 text-white hover:bg-white/20 transition-all">
              Download Report
            </button>
          </div>
        ...
        <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
          <Card_1.Card className="p-6">
            <Card_1.CardHeader title="NPA System Status" subtitle="Real-time connection and service status" action={<div className="flex items-center space-x-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <framer_motion_1.motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }}>
            <Card_1.Card className="p-6">
              <Card_1.CardHeader title="Submission Trends" subtitle="Monthly NPA submissions over time"/>
              <Card_1.CardContent>
                <charts_1.LineChart data={mockData.submissionTrends} height={300}/>
              </Card_1.CardContent>
            </Card_1.Card>
          </framer_motion_1.motion.div>
          ...
          <framer_motion_1.motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.7 }}>
            <Card_1.Card className="p-6">
              <Card_1.CardHeader title="NPA Response Times" subtitle="Average processing time by submission type"/>
              <Card_1.CardContent>
                <charts_1.BarChart data={mockData.responseTime} height={300}/>
              </Card_1.CardContent>
            </Card_1.Card>
          </framer_motion_1.motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }}>
            <Card_1.Card className="p-6">
              <Card_1.CardHeader title="Compliance Status" subtitle="Current compliance rates by category"/>
              <Card_1.CardContent>
        ...
        ].map((item, index) => (<div key={item.label} className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-400">{item.label}</span>
                        <span className="text-white font-medium">{item.value}%</span>
                      </div>
                      <div className="w-full bg-gray-700 rounded-full h-2">
                        <framer_motion_1.motion.div initial={{ width: 0 }} animate={{ width: `${item.value}%` }} transition={{ duration: 1, delay: index * 0.2 }} className={`h-2 rounded-full ${item.color}`}/>
                      </div>
                    </div>))}
                </div>
              </Card_1.CardContent>
          ...
          <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9 }}>
            <Card_1.Card className="p-6">
              <Card_1.CardHeader title="Submission Status Distribution" subtitle="Current status breakdown"/>
              <Card_1.CardContent>
        ...
        <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.0 }}>
          <Card_1.Card className="p-6">
            <Card_1.CardHeader title="Recent NPA Submissions" subtitle="Latest submissions and their status"/>
            <Card_1.CardContent>
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {submissions.map((submission, index) => (<framer_motion_1.motion.tr key={submission.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                        ...
                        <td className="py-3 px-4">
                          <div className="flex space-x-2">
                            ...
                            {submission.status === 'pending' && (<button className="px-3 py-1 bg-green-500 hover:bg-green-600 text-white rounded text-xs transition-colors">
                                Track
                              </button>)}
                          </div>
                        </td>
        ...
        <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1 }}>
          <Card_1.Card className="p-6">
            <Card_1.CardHeader title="Integration Settings" subtitle="Configure NPA integration parameters"/>
            <Card_1.CardContent>
...
exports.default = NPAIntegrationDashboard;
//# sourceMappingURL=npa-integration.js.map