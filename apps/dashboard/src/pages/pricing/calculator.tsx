...
const PriceCalculator: NextPage = () => {
  const [fuelType, setFuelType] = useState('PMS');
  const [basePrice, setBasePrice] = useState(12.50);
  const [volume, setVolume] = useState(1000);
  const [destination, setDestination] = useState('Kumasi');
  ...
  const priceComponents = useMemo(() => {
    ...
    const uppfRates: Record<string, number> = {
      'Kumasi': 8.333,
      'Tamale': 10.125,
      'Cape Coast': 5.625,
      'Bolgatanga': 12.500,
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    className="w-4 h-4 text-primary-500 bg-dark-700 border-white/20 rounded focus:ring-primary-500 focus:ring-2"
                  />
              ...
              <Card className="p-6">
                ...
                <div className="space-y-4">
                  {priceComponents.map((component, index) => (
                    <div key={index} className="flex justify-between items-center p-4 bg-dark-900/30 rounded-lg border border-white/5">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            ...
                            {component.percentage > 0 && (
                              <p className="text-xs text-dark-400">{component.percentage.toFixed(1)}% of base</p>
                            )}
                          </div>
                        </div>
                        <p className="text-sm text-dark-400 mt-1">{component.description}</p>
                        {!component.isFixed && (
                          <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-yellow-500/20 text-yellow-400 border border-yellow-500/30 mt-2">
                            Variable Rate
                          </span>
                        )}
                      </div>
            ...
            <Card className="p-6">
              ...
              <div className="space-y-4">
                {destinationOptions.map((dest) => {
                  const destUppfRates: Record<string, number> = {
                    'Kumasi': 8.333,
                    'Tamale': 10.125,
                    'Cape Coast': 5.625,
                    'Bolgatanga': 12.500,
                  ...
                  return (
                    <div 
                      ...
                      className={`flex justify-between items-center p-3 rounded-lg transition-colors ${
                        isSelected ? 'bg-primary-500/20 border border-primary-500/30' : 'bg-dark-900/30 hover:bg-dark-900/50'
                      }`}
                    >
                      <div>
