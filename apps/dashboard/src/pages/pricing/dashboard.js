...
const PricingDashboard = () => {
    const [pricingMetrics, setPricingMetrics] = (0, react_1.useState)(null);
    const [recentPriceUpdates, setRecentPriceUpdates] = (0, react_1.useState)([]);
    const [priceVarianceData, setPriceVarianceData] = (0, react_1.useState)(null);
    ...
    const loadPricingData = async () => {
        try {
            ...
            const [priceWindows, pbuComponents, settlements] = await Promise.all([
                api_1.pricingService.getPriceWindows(),
                api_1.pricingService.getPBUComponents(),
                api_1.pricingService.getDealerSettlements()
            ]);
            ...
            react_hot_toast_1.toast.success('Pricing data loaded successfully');
        }
        catch (error) {
            console.error('Error loading pricing data:', error);
            setError('Failed to load pricing data');
            react_hot_toast_1.toast.error('Failed to load pricing data');
    ...
    const processPricingMetrics = (priceWindows, pbuComponents, settlements) => {
        ...
        return {
            ...
            averageMargin,
            todaysPriceChanges: priceWindows.filter(w => new Date(w.effectiveDate).toDateString() === new Date().toDateString()).length,
            totalPBUComponents,
    ...
    const processRecentPriceUpdates = (priceWindows) => {
        return priceWindows
    ...
    const processPriceVarianceTrend = (priceWindows) => {
    ...
    if (loading) {
        return (<FuturisticDashboardLayout_1.FuturisticDashboardLayout title="Pricing Management Dashboard" subtitle="Price Build-Up & Window Management">
    ...
    if (error && !pricingMetrics) {
        ...
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <button onClick={loadPricingData} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
              Retry
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <framer_motion_1.motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }}>
            <Card_1.Card className="p-6">
              ...
              {priceVarianceData ? (<charts_1.LineChart data={priceVarianceData} height={300}/>) : (<div className="flex items-center justify-center h-72">
          ...
          <Card_1.Card className="p-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {recentPriceUpdates.length > 0 ? recentPriceUpdates.map((update, index) => (<framer_motion_1.motion.tr key={update.product} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 + index * 0.1 }} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                      <td className="py-3 px-4 font-medium">{update.product}</td>
                      <td className="py-3 px-4 font-bold">₵{update.exPump.toFixed(2)}</td>
                      <td className="py-3 px-4">
                        <span className={`font-medium ${update.change.startsWith('+') ? 'text-green-600' : 'text-red-600'}`}>
                          {update.change}
                        </span>
                      </td>
                    ...
                    </framer_motion_1.motion.tr>)) : (<tr>
                      <td colSpan={5} className="py-8 text-center text-gray-500">
                        {loading ? (<div className="flex items-center justify-center">
                            <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-blue-500 mr-2"></div>
                            Loading price updates...
                          </div>) : ('No recent price updates found')}
                      </td>
                    </tr>)}
                </tbody>
              </table>
...
exports.default = PricingDashboard;
