...
const PricingSettlementsManagement = () => {
  ...
  const loadSettlementsData = async () => {
    try {
      ...
      toast.success('Settlements data loaded successfully');
    } catch (error) {
  ...
  const processSettlement = async (settlementId: string) => {
    try {
      toast.loading('Processing settlement...');
  ...
  const approveSettlement = async (settlementId: string) => {
    try {
      toast.loading('Approving settlement...');
      ...
      toast.success('Settlement approved and completed');
    } catch (error) {
  ...
  const disputeSettlement = async (settlementId: string, reason: string) => {
    try {
      toast.loading('Recording settlement dispute...');
      ...
      toast.success('Settlement dispute recorded');
    } catch (error) {
      toast.error('Failed to record dispute');
    }
  };

  const exportSettlements = async (format: 'csv' | 'pdf' | 'excel') => {
    try {
      toast.loading(`Exporting settlements as ${format.toUpperCase()}...`);
      
      // Simulate export
      await new Promise(resolve => setTimeout(resolve, 2000));
      
      toast.success(`Settlements exported as ${format.toUpperCase()}`);
    } catch (error) {
  ...
  const generateSettlementReport = async () => {
    try {
      toast.loading('Generating comprehensive settlement report...');
      ...
      <div className="space-y-6">
        ...
        >
          ...
          <div className="flex space-x-4">
            <button
              onClick={generateSettlementReport}
            ...
            <div className="relative group">
              ...
              <div className="absolute right-0 top-full mt-2 w-48 glass rounded-lg border border-white/20 bg-white/10 backdrop-blur-md opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10">
                ...
                <button
                  onClick={() => exportSettlements('excel')}
        ...
        {/* Filters */}
        <Card className="p-6">
          <CardHeader title="Settlement Filters" subtitle="Filter settlements by status, period, and dealer" />
          <CardContent>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          >
            <Card className="p-6">
              <CardHeader title="Settlement Trends" subtitle="Monthly settlement amounts over time" />
              <CardContent>
                <LineChart data={mockData.monthlyTrends} height={300} />
              </CardContent>
            </Card>
          </motion.div>
          ...
          >
            <Card className="p-6">
              <CardHeader title="Status Distribution" subtitle="Current settlement status breakdown" />
              <CardContent>
        ...
        >
          <Card className="p-6">
            <CardHeader title="Processing Time Analysis" subtitle="Average settlement processing time by day of week" />
            <CardContent>
              <BarChart data={mockData.processingTime} height={300} />
            </CardContent>
          </Card>
        </motion.div>
        ...
        >
          <Card className="p-6">
            <CardHeader title="Settlement Records" subtitle="Detailed view of all settlement transactions" />
            <CardContent>
        ...
        {selectedSettlement && (
          ...
          >
            <motion.div
              ...
              className="glass rounded-3xl border border-white/20 bg-white/10 backdrop-blur-md max-w-4xl w-full max-h-[90vh] overflow-y-auto"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="p-8">
                ...
                <div className="mb-6">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      ...
                      <tbody>
                        {selectedSettlement.products.map((product, index) => (
                ...
                {/* Dispute Information */}
                {selectedSettlement.status === 'disputed' && selectedSettlement.disputeReason && (
                  <div className="mb-6">
...
export default PricingSettlementsManagement;