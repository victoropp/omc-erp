...
const PriceVarianceAnalysis = () => {
  ...
  const loadVarianceData = async () => {
    try {
      ...
      toast.success('Variance analysis loaded successfully');
    } catch (error) {
  ...
  const exportVarianceReport = async (format: 'csv' | 'pdf' | 'excel') => {
    try {
      toast.loading(`Exporting variance report as ${format.toUpperCase()}...`);
      
      // Simulate export
      await new Promise(resolve => setTimeout(resolve, 2000));
      
      toast.success(`Variance report exported as ${format.toUpperCase()}`);
    } catch (error) {
  ...
  const runVarianceAnalysis = async () => {
    try {
      toast.loading('Running comprehensive variance analysis...');
      
      // Simulate analysis
      await new Promise(resolve => setTimeout(resolve, 3000));
      
      toast.success('Variance analysis completed');
  ...
  const getVarianceColor = (variance: number, threshold: number) => {
    const absVariance = Math.abs(variance);
    if (absVariance >= threshold * 2) return 'text-red-500';
    if (absVariance >= threshold) return 'text-yellow-500';
  ...
  return (
    <FuturisticDashboardLayout>
      <div className="space-y-6">
        ...
        {/* Analysis Filters */}
        <Card className="p-6">
          <CardHeader title="Analysis Filters" subtitle="Configure variance analysis parameters" />
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                ...
                <select
                  value={analysisFilters.period}
                  onChange={(e) => setAnalysisFilters(prev => ({ ...prev, period: e.target.value }))}
              ...
              <div>
                ...
                <select
                  value={analysisFilters.product}
                  onChange={(e) => setAnalysisFilters(prev => ({ ...prev, product: e.target.value }))}
                  className="w-full glass px-3 py-2 rounded-lg border border-white/20 bg-white/10 text-white"
                >
              ...
              <div>
                ...
                <input
                  type="number"
                  step="0.1"
                  value={analysisFilters.threshold}
                  onChange={(e) => setAnalysisFilters(prev => ({ ...prev, threshold: parseFloat(e.target.value) }))}
                  className="w-full glass px-3 py-2 rounded-lg border border-white/20 bg-white/10 text-white"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setAnalysisFilters(prev => ({ ...prev, region: e.target.value }))}
                  className="w-full glass px-3 py-2 rounded-lg border border-white/20 bg-white/10 text-white"
                >
        ...
        >
          <Card className="p-6">
            <CardHeader title="Current Variance Status" subtitle="Real-time variance analysis by product" />
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {mockData.varianceByProduct.map((item, index) => (
                      <motion.tr
                        key={item.product}
                      ...
                      >
                        ...
                        <td className="py-3 px-4">
                          ...
                          {item.status === 'critical' && (
                            <button className="px-3 py-1 bg-red-500 hover:bg-red-600 text-white rounded text-xs transition-colors">
                              Alert
                            </button>
                          )}
                        </td>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          >
            <Card className="p-6">
              <CardHeader title="Historical Variance Trends" subtitle="Product variance over time" />
              <CardContent>
                <LineChart data={mockData.historicalVariance} height={300} />
              </CardContent>
            </Card>
          </motion.div>
          ...
          >
            <Card className="p-6">
              <CardHeader title="Variance Distribution" subtitle="Frequency distribution of variance ranges" />
              <CardContent>
                <BarChart data={mockData.varianceDistribution} height={300} />
              </CardContent>
            </Card>
          </motion.div>
        </div>
        ...
        >
          <Card className="p-6">
            <CardHeader title="Root Cause Analysis" subtitle="Primary factors contributing to price variance" />
            <CardContent>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                  <PieChart data={mockData.rootCauseAnalysis} height={300} />
                </div>
        ...
        >
          <Card className="p-6">
            <CardHeader title="Variance Alerts" subtitle="Active alerts and notifications" />
            <CardContent>
              <div className="space-y-4">
                {alerts.map((alert, index) => (
                  <motion.div
                    ...
                    className={`p-4 rounded-lg border ${
                      alert.severity === 'critical' ? 'bg-red-100 dark:bg-red-900/30 border-red-200 dark:border-red-800' :
