...
interface ProductCategory {
  ...
  name: string;
  code: string;
  type: 'fuel' | 'lubricant' | 'service' | 'accessory';
  description: string;
  parent?: string;
  specifications: {
...
const ProductCategoriesManagement = () => {
  ...
  const handleSpecificationChange = (type: keyof typeof formData.specifications, value: string[]) => {
    setFormData(prev => ({
      ...prev,
      specifications: {
        ...prev.specifications,
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <Badge variant={getTypeBadgeColor(category.type)}>
                          {category.type}
                        </Badge>
                      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Select
                ...
                options={categoryTypes.slice(1)} // Remove 'all' option
                error={errors.type}
              />
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 max-h-32 overflow-y-auto border border-gray-200 dark:border-gray-600 rounded-lg p-3">
                {standardTests[formData.type]?.map((test) => (
                  <label key={test} className="flex items-center gap-2 text-sm">
                    <input
                      type="checkbox"
                      checked={formData.specifications.standardTests.includes(test)}
                      onChange={(e) => {
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 max-h-32 overflow-y-auto border border-gray-200 dark:border-gray-600 rounded-lg p-3">
                {qualityParameters[formData.type]?.map((param) => (
                  <label key={param} className="flex items-center gap-2 text-sm">
                    <input
                      type="checkbox"
                      checked={formData.specifications.qualityParameters.includes(param)}
                      onChange={(e) => {
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 max-h-32 overflow-y-auto border border-gray-200 dark:border-gray-600 rounded-lg p-3">
                {safetyRequirements[formData.type]?.map((req) => (
                  <label key={req} className="flex items-center gap-2 text-sm">
                    <input
                      type="checkbox"
                      checked={formData.specifications.safetyRequirements.includes(req)}
                      onChange={(e) => {
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 max-h-32 overflow-y-auto border border-gray-200 dark:border-gray-600 rounded-lg p-3">
                {regulatoryRequirements[formData.type]?.map((req) => (
                  <label key={req} className="flex items-center gap-2 text-sm">
                    <input
                      type="checkbox"
                      checked={formData.specifications.regulatoryRequirements.includes(req)}
                      onChange={(e) => {
                        const reqs = e.target.checked
