...
const PayrollPage: NextPage = () => {
  ...
  const [modalType, setModalType] = useState<'period' | 'entry' | 'bulk'>('period');
  ...
  const loadData = async () => {
    try {
      ...
      const [periodsData, employeesData] = await Promise.all([
        hrService.getPayrollPeriods?.() || Promise.resolve([]),
  ...
  const loadPayrollEntries = async (periodId: string) => {
    try {
      const data = await hrService.getPayrollEntries?.(periodId) || [];
  ...
  const generateMockPeriods = (): PayrollPeriod[] => [
    {
      id: 'p1',
      period: '2024-01',
  ...
  const generateMockEntries = (periodId: string): PayrollEntry[] => [
    {
      id: 'pe1',
      periodId,
      employee: {
  ...
  const handleProcessPayroll = async () => {
    ...
    try {
      await hrService.processPayroll({ periodId: currentPeriod.id });
  ...
  return (
    <FuturisticDashboardLayout>
      <div className="space-y-8">
              ...
              <div className="flex items-center space-x-4">
                <Select
                  ...
                  onChange={(value) => {
                    ...
                    if (period) loadPayrollEntries(period.id);
                  }}
                ...
                />
                {currentPeriod && (
                  <Badge variant={getStatusColor(currentPeriod.status)} className="capitalize">
                ...
                {currentPeriod && (
                  <>
                    ...
                    {currentPeriod.processedAt && (
                      <p>Processed: {new Date(currentPeriod.processedAt).toLocaleString()}</p>
                    )}
                  </>
                )}
              </div>
        ...
        <Card>
          <CardHeader title={`Payroll Entries - ${currentPeriod?.period || 'Select Period'}`} />
          <CardContent>
            {loading ? (
              ...
              <Table
                ...
                data={filteredEntries.map(entry => ({
                  ...
                  salary: (
                    <div>
                      ...
                      {entry.overtimeHours > 0 && (
                        <p className="text-xs text-blue-400">{entry.overtimeHours}h OT</p>
                      )}
                    </div>
                  ),
                  allowances: (
                  ...
                  gross: (
                    <p className="font-medium text-white">GHS {entry.grossPay.toLocaleString()}</p>
                  ),
                  net: (
                    <p className="font-bold text-green-400">GHS {entry.netPay.toLocaleString()}</p>
                  ),
                  status: (
              ...
              <div>
                ...
                <div className="space-y-3">
                  {payrollSummary.departmentBreakdown.map(dept => (
                    <div key={dept.department} className="flex items-center justify-between p-3 bg-dark-700 rounded-lg">
                      <div>
        ...
        <Modal
          isOpen={isModalOpen && modalType === 'period'}
