...
export class TestDataFactory {
  private static readonly DEFAULT_TENANT_ID = 'test-tenant-123';
  private static readonly DEFAULT_USER_ID = 'test-user-123';
  ...
  static createTestClaim(overrides: Partial<any> = {}) {
    ...
    const deliveryId = overrides.deliveryId || uuidv4();
    ...
    return {
      ...
      status: overrides.status || UPPFClaimStatus.DRAFT,
  ...
  static createTestCreateClaimDto(overrides: Partial<any> = {}) {
    return {
      ...
      deliveryId: overrides.deliveryId || uuidv4(),
      routeId: overrides.routeId || 'route-accra-tema',
  ...
  static createTestPricingWindow(overrides: Partial<any> = {}) {
    ...
    return {
      ...
      status: overrides.status || PricingWindowStatus.DRAFT,
  ...
  static createTestDealerSettlement(overrides: Partial<any> = {}) {
    ...
    return {
      ...
      status: overrides.status || DealerSettlementStatus.PENDING,
  ...
  // Utility Functions
  static generateGPSPoints(startLat: number, startLng: number, totalKm: number, pointsCount: number = 10) {
    const points = [];
  ...
  static createConcurrentTestScenario() {
    return {
      ...
      pricingWindows: Array.from({ length: 12 }, (_, i) => 
        this.createTestPricingWindow({ windowId: `PW-2024-W${String(i + 1).padStart(2, '0')}` })
      ),
      dealers: Array.from({ length: 50 }, (_, i) => 
        this.createTestDealer({ dealerCode: `DLR-${String(i + 1).padStart(3, '0')}` })
      ),
    };
  }
