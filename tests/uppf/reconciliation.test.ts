...
describe('UPPF Three-Way Reconciliation Comprehensive Tests', () => {
  ...
  describe('Perfect Reconciliation Scenarios', () => {
    it('should pass reconciliation with identical values', async () => {
      ...
      const claimData = TestDataFactory.createTestCreateClaimDto({
  ...
  describe('GPS Distance Integration', () => {
    it('should perform distance reconciliation with GPS data', async () => {
      ...
      jest.spyOn(service as any, 'calculateGPSDistance').mockReturnValue(185);
    ...
    it('should flag significant GPS distance discrepancies', async () => {
      ...
      const claimData = TestDataFactory.createTestCreateClaimDto({
        kmActual: 180,
        gpsTrace: TestDataFactory.generateGPSPoints(5.6037, -0.1870, 250, 20), // 70km difference
      });

      jest.spyOn(service as any, 'calculateGPSDistance').mockReturnValue(250);
    ...
    it('should calculate tolerance based on claimed distance', async () => {
      ...
      for (const testCase of testCases) {
        ...
        const claimData = TestDataFactory.createTestCreateClaimDto({
          ...
          gpsTrace: TestDataFactory.generateGPSPoints(5.6037, -0.1870, testCase.gps, 10),
        });

        jest.spyOn(service as any, 'calculateGPSDistance').mockReturnValue(testCase.gps);
  ...
  describe('Complex Multi-Variance Scenarios', () => {
    it('should handle simultaneous volume and distance variances', async () => {
      ...
      const claimData = TestDataFactory.createTestCreateClaimDto({
        ...
        kmActual: 180,
        gpsTrace: TestDataFactory.generateGPSPoints(5.6037, -0.1870, 220, 20), // 40km variance
      });

      jest.spyOn(service as any, 'calculateGPSDistance').mockReturnValue(220);
    ...
    it('should prioritize variances by severity', async () => {
      ...
      const claimData = TestDataFactory.createTestCreateClaimDto({
        ...
        kmActual: 180,
        gpsTrace: TestDataFactory.generateGPSPoints(5.6037, -0.1870, 185, 20), // Minor GPS variance
      });
    ...
    it('should provide comprehensive reconciliation summary', async () => {
      ...
      const claimData = TestDataFactory.createTestCreateClaimDto({
        litresMoved: 39800,
        kmActual: 180,
        gpsTrace: TestDataFactory.generateGPSPoints(5.6037, -0.1870, 178, 20),
      });

      jest.spyOn(service as any, 'calculateGPSDistance').mockReturnValue(178);
  ...
  describe('Data Quality and Validation', () => {
    ...
    it('should validate data type consistency', async () => {
      ...
      const delivery = TestDataFactory.createTestDelivery({
        litresLoaded: 40000,
        litresReceived: '39950' as any, // String instead of number
      });
  ...
  describe('Edge Cases and Boundary Conditions', () => {
    ...
    it('should handle zero distance claims', async () => {
      ...
      jest.spyOn(service as any, 'calculateGPSDistance').mockReturnValue(0);
  ...
  describe('Performance and Efficiency', () => {
    it('should complete reconciliation within performance threshold', async () => {
      ...
      const delivery = TestDataFactory.createTestDelivery();
      const claimData = TestDataFactory.createTestCreateClaimDto({
        gpsTrace: TestDataFactory.generateGPSPoints(5.6037, -0.1870, 180, 1000), // Large GPS trace
      });
    ...
    it('should handle concurrent reconciliation requests', async () => {
      ...
      const concurrentRequests = Array.from({ length: 100 }, (_, i) => 
        TestDataFactory.createTestCreateClaimDto({
      ...
      const results = await Promise.all(
        concurrentRequests.map(claimData => 
          service.performThreeWayReconciliation(delivery, claimData)
        )
      );
