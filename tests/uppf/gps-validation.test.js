...
describe('GPS Validation and Route Analysis', () => {
    ...
    describe('GPS Distance Calculation', () => {
        ...
        it('should handle large-scale GPS trace efficiently', () => {
            // Arrange
            const largeTrace = test_data_factory_1.TestDataFactory.generateGPSPoints(5.6037, -0.1870, 500, 1000); // 500km, 1000 points
    ...
    describe('GPS Trace Creation and Storage', () => {
        it('should create GPS trace with complete metadata', async () => {
            ...
            const gpsPoints = test_data_factory_1.TestDataFactory.generateGPSPoints(5.6037, -0.1870, 180, 50);
            const expectedTrace = test_data_factory_1.TestDataFactory.createTestGPSTrace({
                deliveryId,
                totalKm: 180,
        ...
        it('should handle GPS trace with timestamps in different timezones', async () => {
            ...
            gpsTraceRepository.create.mockReturnValue(expectedTrace);
            gpsTraceRepository.save.mockResolvedValue(expectedTrace);
            jest.spyOn(service, 'calculateGPSDistance').mockReturnValue(2.22);
            // Act
    ...
    describe('Distance Validation in Reconciliation', () => {
        it('should flag distance variance within tolerance', async () => {
            ...
            const claimWithGPS = test_data_factory_1.TestDataFactory.createTestCreateClaimDto({
                kmActual: 180,
                gpsTrace: test_data_factory_1.TestDataFactory.generateGPSPoints(5.6037, -0.1870, 185, 20), // 5km difference
            });
            jest.spyOn(service, 'calculateGPSDistance').mockReturnValue(185); // GPS shows 185km
            // Act
        ...
        it('should flag distance variance exceeding tolerance', async () => {
            ...
            const claimWithGPS = test_data_factory_1.TestDataFactory.createTestCreateClaimDto({
                kmActual: 180,
                gpsTrace: test_data_factory_1.TestDataFactory.generateGPSPoints(5.6037, -0.1870, 220, 20), // 40km difference
            });
            jest.spyOn(service, 'calculateGPSDistance').mockReturnValue(220); // GPS shows 220km
            // Act
            const result = await service.performThreeWayReconciliation(delivery, claimWithGPS);
            // Assert
        ...
        it('should handle claims without GPS trace gracefully', async () => {
            // Arrange
            const delivery = test_data_factory_1.TestDataFactory.createTestDelivery();
            const claimWithoutGPS = test_data_factory_1.TestDataFactory.createTestCreateClaimDto({
                kmActual: 180,
                gpsTrace: undefined,
            });
            // Act
        ...
        it('should calculate distance variance for empty GPS trace', async () => {
            // Arrange
            const delivery = test_data_factory_1.TestDataFactory.createTestDelivery();
            const claimWithEmptyGPS = test_data_factory_1.TestDataFactory.createTestCreateClaimDto({
                kmActual: 180,
                gpsTrace: [],
            });
            // Act
    ...
    describe('GPS Data Quality Assessment', () => {
        it('should assess GPS data quality based on point density', () => {
            ...
            const sparseTrace = test_data_factory_1.TestDataFactory.generateGPSPoints(5.6037, -0.1870, 180, 5); // Only 5 points for 180km
            // Act
    ...
    describe('Performance and Scale', () => {
        it('should process GPS trace with 10,000 points efficiently', () => {
            // Arrange
            const largeTrace = test_data_factory_1.TestDataFactory.generateGPSPoints(5.6037, -0.1870, 1000, 10000);
        ...
        it('should handle concurrent GPS processing', async () => {
            // Arrange
            const concurrentTraces = Array.from({ length: 100 }, (_, i) => test_data_factory_1.TestDataFactory.generateGPSPoints(5.6037 + (i * 0.001), -0.1870 + (i * 0.001), 100, 50));
            mockedGeolib.getDistance.mockReturnValue(200);
            const startTime = Date.now();
            // Act
            const results = await Promise.all(concurrentTraces.map(trace => Promise.resolve(service['calculateGPSDistance'](trace))));
            const endTime = Date.now();
            const executionTime = endTime - startTime;
            // Assert
...
//# sourceMappingURL=gps-validation.test.js.map