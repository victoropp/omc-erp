...
const auth_module_1 = require("../../services/auth-service/src/auth/auth.module");
const pricing_module_1 = require("../../services/pricing-service/src/pricing/pricing.module");
...
describe('Auth and Pricing Integration Tests', () => {
    ...
    const testUser = {
        ...
        role: shared_types_1.UserRole.PRICING_MANAGER,
    };
    ...
    beforeAll(async () => {
        const moduleFixture = await testing_1.Test.createTestingModule({
            imports: [
                typeorm_1.TypeOrmModule.forRoot({
                    type: 'sqlite',
                    database: ':memory:',
                    entities: [database_1.User, database_1.PricingWindow, database_1.StationPrice],
                    synchronize: true,
                    logging: false,
                }),
        ...
        await app.init();
        connection = app.get((0, typeorm_1.getConnectionToken)());
    });
    afterAll(async () => {
    ...
    describe('Complete Authentication and Pricing Workflow', () => {
        it('should complete full user registration to pricing window creation workflow', async () => {
            ...
            expect(profileResponse.body.role).toBe(shared_types_1.UserRole.PRICING_MANAGER);
            // Step 3: Create pricing window with authenticated user
            const createWindowResponse = await (0, supertest_1.default)(app.getHttpServer())
            ...
            expect(createWindowResponse.body).toHaveProperty('status', shared_types_1.PricingWindowStatus.DRAFT);
            expect(createWindowResponse.body).toHaveProperty('createdBy');
            // Step 4: Verify user can retrieve their created pricing window
            const getWindowResponse = await (0, supertest_1.default)(app.getHttpServer())
        ...
        it('should enforce role-based access control for pricing operations', async () => {
            ...
            const operatorUser = {
                ...
                role: shared_types_1.UserRole.OPERATOR,
            };
            const registerResponse = await (0, supertest_1.default)(app.getHttpServer())
                .post('/auth/register')
                .send(operatorUser)
            ...
            const adminRegisterResponse = await (0, supertest_1.default)(app.getHttpServer())
                .post('/auth/register')
                .send(adminUser)
                .expect(201);
        ...
        it('should maintain tenant isolation across authentication and pricing', async () => {
            ...
            const tenant1Response = await (0, supertest_1.default)(app.getHttpServer())
                .post('/auth/register')
                .send(tenant1User)
                .expect(201);
            const tenant2Response = await (0, supertest_1.default)(app.getHttpServer())
                .post('/auth/register')
                .send(tenant2User)
                .expect(201);
            ...
            const tenant1WindowsResponse = await (0, supertest_1.default)(app.getHttpServer())
                .get('/pricing/windows')
                .set('Authorization', `Bearer ${tenant1Token}`)
                .expect(200);
            ...
            const tenant2WindowsResponse = await (0, supertest_1.default)(app.getHttpServer())
                .get('/pricing/windows')
                .set('Authorization', `Bearer ${tenant2Token}`)
                .expect(200);
        ...
        it('should handle concurrent user operations and pricing calculations', async () => {
            ...
            const registrationPromises = users.map(user => (0, supertest_1.default)(app.getHttpServer())
                .post('/auth/register')
                .send(user));
            ...
            const windowPromises = registrationResponses.map((response, index) => (0, supertest_1.default)(app.getHttpServer())
                .post('/pricing/windows')
    ...
    describe('Database Transaction Consistency', () => {
        it('should maintain data consistency across auth and pricing operations', async () => {
            ...
            const pricingWindow = await connection
                .getRepository(database_1.PricingWindow)
            ...
            await connection
                .getRepository(database_1.User)
                .update({ id: userId }, { status: shared_types_1.UserStatus.SUSPENDED });
        ...
        it('should rollback pricing window creation on authentication failure', async () => {
            ...
            await connection
                .getRepository(database_1.User)
                .update({ email: testUser.email }, { status: shared_types_1.UserStatus.INACTIVE });
    ...
    describe('Performance and Load Testing', () => {
        it('should handle high-concurrency authentication and pricing operations', async () => {
            ...
            const operations = Array.from({ length: concurrencyLevel }, async (_, i) => {
                ...
                const token = registerResponse.body.accessToken;
                // Create pricing window
                const windowResponse = await (0, supertest_1.default)(app.getHttpServer())
                    .post('/pricing/windows')
                    .set('Authorization', `Bearer ${token}`)
                    .send({
                    ...testPricingWindow,
    ...
    describe('Security Integration Tests', () => {
        ...
        it('should prevent privilege escalation through token manipulation', async () => {
            ...
            const registerResponse = await (0, supertest_1.default)(app.getHttpServer())
                ...
                .send({
                ...testUser,
                role: shared_types_1.UserRole.OPERATOR, // Lower privilege role
            })
                .expect(201);
            const token = registerResponse.body.accessToken;
            ...
            expect(user.role).toBe(shared_types_1.UserRole.OPERATOR);
        });
    });
});
//# sourceMappingURL=auth-pricing-integration.test.js.map