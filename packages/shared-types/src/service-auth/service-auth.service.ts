...
export class ServiceAuthService implements IServiceAuthService {
  ...
  async generateApiKey(serviceName: string): Promise<ServiceApiKey> {
    try {
      ...
      this.logger.log(`Generated API key for service: ${serviceName}`);
  ...
  async validateServiceToken(token: string): Promise<ServiceTokenPayload | null> {
    try {
      ...
      if (cachedPayload) {
        ...
        await this.cacheService.delete(`${this.CACHE_PREFIX}token:${tokenHash}`);
      }
  ...
  async authenticateService(request: ServiceAuthRequest): Promise<ServiceAuthResponse> {
    try {
      ...
      if (serviceIdentity.id !== request.serviceId) {
        return {
          success: false,
  ...
  async listServiceApiKeys(serviceName?: string): Promise<ServiceApiKey[]> {
    ...
    // For now, return empty array as cache doesn't support pattern scanning
    this.logger.warn('listServiceApiKeys not fully implemented - requires database backend');
    return [];
  }

  async updateServicePermissions(serviceId: string, permissions: string[]): Promise<boolean> {
    try {
      // This would update the service's permissions in the database
