...
let InventoryService = InventoryService_1 = class InventoryService {
     ...
     */
    async processStockReceipt(data) {
        ...
        try {
            ...
            const receipt = await queryRunner.manager.query(`INSERT INTO stock_receipts (
        ...
        RETURNING *`, [
            ...
            for (const item of data.items) {
                // Create receipt item
                await queryRunner.manager.query(`INSERT INTO stock_receipt_items (
            ...
            temperature_celsius, density_reading, quality_status
                ...
                await this.createInventoryMovement(queryRunner, 'RECEIPT', data.stationId, item.productId, item.tankId, item.deliveredQuantity, item.unitCost, 'STOCK_RECEIPT', receipt[0].id);
                // Update stock levels
                await this.updateStockLevel(queryRunner, data.stationId, item.productId, item.tankId, item.deliveredQuantity);
     ...
     */
    async processFuelSale(transactionData) {
        const queryRunner = this.dataSource.createQueryRunner();
        await queryRunner.connect();
        await queryRunner.startTransaction();
        try {
            ...
            await this.createInventoryMovement(queryRunner, 'SALE', stationId, productId, tankId, -quantityLiters, unitPrice, 'FUEL_TRANSACTION', transactionData.id);
            // Update stock level
     ...
     */
    async processTankReading(reading) {
        try {
            // Store tank reading
            await this.dataSource.query(`INSERT INTO tank_readings (
          ...
          water_level_mm, density_reading, sensor_status, alert_triggered
            ...
            await this.dataSource.query(`UPDATE stock_levels 
        SET current_quantity = $1, updated_at = CURRENT_TIMESTAMP
        WHERE station_id = $2 AND product_id = $3 AND tank_id = $4`, [reading.volumeLiters, reading.stationId, reading.productId, reading.tankId]);
            // Check for alerts
    ...
    async performStockTake(data) {
        ...
        try {
            ...
            const stockTake = await queryRunner.manager.query(`INSERT INTO stock_takes (
          stock_take_number, stock_take_date, station_id, status
        ) VALUES ($1, $2, $3, $4)
        RETURNING *`, [stockTakeNumber, new Date(), data.stationId, 'IN_PROGRESS']);
            let totalVarianceValue = 0;
            // Process each item
            for (const detail of data.details) {
                // Get system quantity
                const systemStock = await queryRunner.manager.query(`SELECT current_quantity FROM stock_levels 
          WHERE station_id = $1 AND product_id = $2 AND 
          ($3::uuid IS NULL OR tank_id = $3)`, [data.stationId, detail.productId, detail.tankId]);
                ...
                const product = await queryRunner.manager.query(`SELECT p.*, 
          ...
          FROM products p WHERE p.id = $2`, [data.stationId, detail.productId]);
                const unitCost = product[0]?.last_cost || 0;
                const varianceValue = varianceQuantity * unitCost;
                totalVarianceValue += varianceValue;
                // Create stock take detail
                await queryRunner.manager.query(`INSERT INTO stock_take_details (
            ...
            unit_cost, variance_value, notes
                ...
                if (Math.abs(varianceQuantity) > 0.01) {
                    await this.createInventoryAdjustment(queryRunner, data.stationId, detail.productId, detail.tankId, varianceQuantity, unitCost, stockTake[0].id);
                }
            }
            // Update stock take with total variance
            await queryRunner.manager.query(`UPDATE stock_takes 
        SET total_variance_value = $1, status = 'COMPLETED', completed_at = CURRENT_TIMESTAMP
        WHERE id = $2`, [totalVarianceValue, stockTake[0].id]);
            await queryRunner.commitTransaction();
     ...
     */
    async getInventoryDashboard(stationId) {
        try {
            // Current stock levels
            const stockLevels = await this.dataSource.query(`SELECT 
          sl.*, 
          p.product_name,
          p.product_type,
        ...
        ORDER BY s.name, p.product_name`, [stationId]);
            // Recent movements
            const recentMovements = await this.dataSource.query(`SELECT 
          im.*,
          p.product_name,
          s.name as station_name
        ...
        LIMIT 50`, [stationId]);
            // Pending receipts
            const pendingReceipts = await this.dataSource.query(`SELECT 
          sr.*,
          s.name as station_name,
        ...
        ORDER BY sr.receipt_date DESC`, [stationId]);
            // Stock alerts
            const stockAlerts = await this.dataSource.query(`SELECT 
          sl.*,
          p.product_name,
          s.name as station_name,
          CASE 
        ...
        ORDER BY sl.current_quantity / NULLIF(p.reorder_level, 0)`, [stationId]);
            // Tank readings (last 24 hours)
            const tankReadings = await this.dataSource.query(`SELECT 
          tr.*,
          t.tank_number,
          p.product_name,
          s.name as station_name
        ...
        ORDER BY tr.time DESC`, [stationId]);
            return {
                stockLevels,
                recentMovements,
                pendingReceipts,
    ...
    async processAutomaticReorders() {
        try {
            ...
            const itemsToReorder = await this.dataSource.query(`SELECT 
          sl.*,
          p.product_name,
          p.reorder_quantity,
          s.name as station_name,
     ...
     */
    async createInventoryMovement(queryRunner, movementType, stationId, productId, tankId, quantity, unitCost, sourceType, sourceId) {
        // Get current balance
        const currentStock = await queryRunner.manager.query(`SELECT current_quantity FROM stock_levels 
      WHERE station_id = $1 AND product_id = $2 
      AND ($3::uuid IS NULL OR tank_id = $3)`, [stationId, productId, tankId]);
        const balanceBefore = currentStock[0]?.current_quantity || 0;
        const balanceAfter = balanceBefore + quantity;
        await queryRunner.manager.query(`INSERT INTO inventory_movements (
     ...
     * Update stock level
     */
    async updateStockLevel(queryRunner, stationId, productId, tankId, quantityChange) {
        await queryRunner.manager.query(`INSERT INTO stock_levels (
      ...
      DO UPDATE SET 
        ...
        updated_at = CURRENT_TIMESTAMP`, [stationId, productId, tankId, quantityChange]);
    }
    /**
     * Check and create alerts for tank readings
     */
    async checkTankAlerts(reading) {
        const alerts = [];
        // Check low level
        ...
        for (const alert of alerts) {
            await this.dataSource.query(`INSERT INTO alerts (
     ...
     * Helper methods
     */
    async generateReceiptNumber(queryRunner) {
        const date = new Date();
        const prefix = `SR-${date.getFullYear()}${(date.getMonth() + 1).toString().padStart(2, '0')}`;
        const result = await queryRunner.manager.query(`SELECT COUNT(*) as count FROM stock_receipts WHERE receipt_number LIKE $1`, [`${prefix}%`]);
        const sequence = (parseInt(result[0].count) + 1).toString().padStart(4, '0');
        return `${prefix}-${sequence}`;
    }
    async generateStockTakeNumber(queryRunner) {
        const date = new Date();
        const prefix = `ST-${date.getFullYear()}${(date.getMonth() + 1).toString().padStart(2, '0')}`;
        const result = await queryRunner.manager.query(`SELECT COUNT(*) as count FROM stock_takes WHERE stock_take_number LIKE $1`, [`${prefix}%`]);
        const sequence = (parseInt(result[0].count) + 1).toString().padStart(4, '0');
        return `${prefix}-${sequence}`;
    }
    async checkLowStock(queryRunner, stationId, productId, tankId) {
        const result = await queryRunner.manager.query(`SELECT 
        sl.current_quantity,
        p.minimum_stock,
        p.reorder_level
      FROM stock_levels sl
      ...
      AND ($3::uuid IS NULL OR sl.tank_id = $3)`, [stationId, productId, tankId]);
        if (result[0]) {
    ...
    async checkReorderLevel(queryRunner, stationId, productId) {
    ...
    async createInventoryAdjustment(queryRunner, stationId, productId, tankId, varianceQuantity, unitCost, stockTakeId) {
        // Create inventory movement for adjustment
        await this.createInventoryMovement(queryRunner, 'ADJUSTMENT', stationId, productId, tankId, varianceQuantity, unitCost, 'STOCK_TAKE', stockTakeId);
        // Update stock level
    ...
    async createInventoryJournalEntry(queryRunner, receipt, items) {
...
], InventoryService.prototype, "server", void 0);
__decorate([
    (0, schedule_1.Cron)(schedule_1.CronExpression.EVERY_HOUR),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], InventoryService.prototype, "processAutomaticReorders", null);
