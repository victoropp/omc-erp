...
var UPPFSettlementsService_1;
var _a;
Object.defineProperty(exports, "__esModule", { value: true });
exports.UPPFSettlementsService = void 0;
...
let UPPFSettlementsService = UPPFSettlementsService_1 = class UPPFSettlementsService {
    ...
    httpService;
    configService;
    logger = new common_1.Logger(UPPFSettlementsService_1.name);
    constructor(settlementRepository, claimRepository, dataSource, eventEmitter, httpService, configService) {
        ...
        try {
            ...
            const eligibleClaims = await this.claimRepository.find({
                where: {
                    ...
                    status: 'approved',
                    ...(dto.claimIds && { id: (0, typeorm_2.In)(dto.claimIds) }),
                },
                relations: ['reconciliation'],
            });
            if (eligibleClaims.length === 0) {
                throw new common_1.BadRequestException('No eligible claims found for settlement');
            }
            // Calculate settlement amounts
            const calculationResult = await this.calculateSettlementAmounts(eligibleClaims, {
                includePenalties: true,
                includePerformanceBonuses: true,
                settlementType: dto.settlementType || uppf_entities_1.SettlementType.REGULAR,
            });
            ...
            const settlement = this.settlementRepository.create({
                ...
                status: uppf_entities_1.SettlementStatus.PENDING,
            ...
            // Save settlement
            const savedSettlement = await queryRunner.manager.save(uppf_entities_1.UPPFSettlement, settlement);
            // Update claims with settlement reference
            await queryRunner.manager.update(uppf_entities_1.UPPFClaim, { id: (0, typeorm_2.In)(eligibleClaims.map(c => c.id)) }, {
     ...
     */
    async processSettlement(settlementId, processData) {
        try {
            ...
            settlement.status = uppf_entities_1.SettlementStatus.PROCESSING;
     ...
     */
    async reconcileSettlement(settlementId, reconcileData) {
        try {
            ...
            settlement.status = Math.abs(variancePercentage) <= 0.1 ?
                uppf_entities_1.SettlementStatus.RECONCILED : uppf_entities_1.SettlementStatus.VARIANCE_DETECTED;
     ...
     */
    async calculateSettlementAmounts(claims, options) {
     ...
     */
    async calculatePenalties(claim) {
        let penalties = 0;
     ...
     */
    async calculatePerformanceBonuses(claim) {
        let bonuses = 0;
     ...
     */
    async generateSettlementReport(settlementId, format = 'pdf', generatedBy) {
        try {
            ...
            const response = await (0, rxjs_1.firstValueFrom)(this.httpService.post('/reports-service/generate', {
     ...
     */
    async submitToNPA(settlementId, submittedBy) {
        try {
            ...
            if (settlement.status !== uppf_entities_1.SettlementStatus.RECONCILED) {
                throw new common_1.BadRequestException('Settlement must be reconciled before NPA submission');
            }
            ...
            const response = await (0, rxjs_1.firstValueFrom)(this.httpService.post('/npa-service/settlements/submit', submissionData, {
                headers: { 'X-Service-Authorization': this.configService.get('SERVICE_TOKEN') },
            }));
            const npaSubmission = response.data;
            ...
            await this.settlementRepository.update(settlement.id, {
                ...
                status: uppf_entities_1.SettlementStatus.SUBMITTED_TO_NPA,
                lastModifiedAt: new Date(),
     ...
     */
    async getSettlementStatistics(filters = {}) {
        ...
        // Performance metrics
        const reconciledCount = settlements.filter(s => s.status === uppf_entities_1.SettlementStatus.RECONCILED).length;
    ...
    // Helper methods
    async generateSettlementReference(windowId) {
        const year = new Date().getFullYear();
        const month = String(new Date().getMonth() + 1).padStart(2, '0');
        const count = await this.settlementRepository.count({
            where: {
                settlementDate: (0, typeorm_2.Between)(new Date(year, new Date().getMonth(), 1), new Date(year, new Date().getMonth() + 1, 0)),
            },
        });
        return `UPPF-SETTLEMENT-${year}${month}-${String(count + 1).padStart(4, '0')}`;
    }
    getSubmissionDelay(claim) {
    ...
    getSubmissionAdvance(claim) {
        // Calculate days submitted early
        return 0; // Placeholder
    }
    analyzeVarianceCauses(variancePercentage) {
        const causes = [];
    ...
    getVarianceRecommendations(variancePercentage) {
    ...
    async prepareSettlementReportData(settlement) {
        ...
        return {
            settlement,
            claims,
            calculationDetails: settlement.calculationDetails,
            complianceMetrics: {
    ...
    async prepareNPASubmissionData(settlement) {
        return {
            settlementId: settlement.settlementId,
            windowId: settlement.windowId,
...
exports.UPPFSettlementsService = UPPFSettlementsService;
exports.UPPFSettlementsService = UPPFSettlementsService = UPPFSettlementsService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(uppf_entities_1.UPPFSettlement)),
    __param(1, (0, typeorm_1.InjectRepository)(uppf_entities_1.UPPFClaim)),
...
//# sourceMappingURL=uppf-settlements.service.js.map