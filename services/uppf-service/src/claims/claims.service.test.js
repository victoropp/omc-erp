...
describe('ClaimsService', () => {
    ...
    describe('createClaim', () => {
        ...
        it('should create GPS trace when GPS data provided', async () => {
            // Arrange
            const createGPSTraceSpy = jest.spyOn(service, 'createGPSTrace')
            ...
            // Assert
            expect(createGPSTraceSpy).toHaveBeenCalledWith(mockDeliveryId, mockCreateClaimDto.gpsTrace, mockTenantId);
        });
        it('should flag variances and set status to UNDER_REVIEW', async () => {
            ...
            const claimWithVariances = { ...mockUPPFClaim, status: shared_types_1.UPPFClaimStatus.UNDER_REVIEW };
            queryRunner.manager.save.mockResolvedValue(claimWithVariances);
            // Act
            const result = await service.createClaim(mockCreateClaimDto, mockTenantId, mockUserId);
            // Assert
            expect(result.status).toBe(shared_types_1.UPPFClaimStatus.UNDER_REVIEW);
    ...
    describe('batchSubmitClaims', () => {
        ...
        it('should batch submit claims successfully', async () => {
            ...
            uppfClaimRepository.update.mockResolvedValue({ affected: 3 });
            ...
            expect(uppfClaimRepository.update).toHaveBeenCalledWith({ windowId: mockWindowId, tenantId: mockTenantId, status: shared_types_1.UPPFClaimStatus.READY_TO_SUBMIT }, expect.objectContaining({
    ...
    describe('getVarianceDashboard', () => {
        it('should generate variance dashboard data', async () => {
            // Arrange
            const claimsData = [
                { ...mockUPPFClaim, status: shared_types_1.UPPFClaimStatus.PAID, amountDue: 1000, amountPaid: 1000, submittedAt: new Date('2024-01-01') },
                { ...mockUPPFClaim, id: 'claim-2', status: shared_types_1.UPPFClaimStatus.PAID, amountDue: 1500, amountPaid: 1200, submittedAt: new Date('2024-01-15') }, // Short pay
                { ...mockUPPFClaim, id: 'claim-3', status: shared_types_1.UPPFClaimStatus.SUBMITTED, amountDue: 2000, submittedAt: new Date('2024-01-20') }, // Pending
            ];
            const queryBuilder = {
        ...
        it('should categorize claims by aging correctly', async () => {
            ...
            const agingClaims = [
                { ...mockUPPFClaim, status: shared_types_1.UPPFClaimStatus.SUBMITTED, amountDue: 1000, submittedAt: oldDate },
            ];
            const queryBuilder = {
                where: jest.fn().mockReturnThis(),
                andWhere: jest.fn().mockReturnThis(),
    ...
    describe('checkAgingClaims (Cron Job)', () => {
        it('should emit alerts for aging claims', async () => {
            ...
            const agingClaims = [
                { ...mockUPPFClaim, status: shared_types_1.UPPFClaimStatus.SUBMITTED, submittedAt: agingDate },
            ];
        ...
        it('should not emit alerts for recent claims', async () => {
            // Arrange
            const recentClaims = [
                { ...mockUPPFClaim, status: shared_types_1.UPPFClaimStatus.SUBMITTED, submittedAt: new Date() }, // Today
            ];
    ...
    describe('Helper Methods', () => {
        ...
        it('should create GPS trace successfully', async () => {
            ...
            expect(gpsTraceRepository.create).toHaveBeenCalledWith(expect.objectContaining({
                deliveryId: mockDeliveryId,
                tenantId: mockTenantId,
                totalKm: 180,
    ...
    describe('Performance and Scale Tests', () => {
        ...
        it('should handle concurrent claim creation', async () => {
            ...
            const concurrentClaims = Array(5).fill(null).map((_, i) => service.createClaim({
                ...mockCreateClaimDto,
        ...
        it('should efficiently process batch submissions', async () => {
            ...
            uppfClaimRepository.update.mockResolvedValue({ affected: 100 });
            const startTime = Date.now();
            // Act
            const result = await service.batchSubmitClaims(mockWindowId, mockTenantId, mockUserId);
            const endTime = Date.now();
...
//# sourceMappingURL=claims.service.test.js.map