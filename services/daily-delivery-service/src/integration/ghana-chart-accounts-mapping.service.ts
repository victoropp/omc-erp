...
@Injectable()
export class GhanaChartAccountsMappingService {
  private readonly logger = new Logger(GhanaChartAccountsMappingService.name);
  
  // Ghana-specific Chart of Accounts structure
  private readonly GHANA_CHART_OF_ACCOUNTS: GhanaAccountMapping[] = [
    // ASSETS
  ...
  private readonly TRANSACTION_MAPPINGS: TransactionMapping[] = [
    {
  ...
  getTransactionMapping(transactionType: string, productType?: ProductGrade, deliveryType?: DeliveryType): TransactionMapping {
    ...
    const mapping: TransactionMapping = { ...baseMapping };
   ...
   */
  buildCustomerInvoiceJournalLines(
    productType: ProductGrade,
    subtotal: number,
    vatAmount: number,
    marginAmounts: { [key: string]: number },
    customerId: string,
  ): any[] {
    const lines = [];
    let lineNumber = 1;
   ...
   */
  async initializeGhanaChartOfAccounts(tenantId: string): Promise<boolean> {
    try {
   ...
   */
  async getAccountBalanceWithCompliance(accountCode: string, asOfDate?: Date): Promise<any> {
    try {
      const response = await firstValueFrom(
        this.httpService.get(`/accounting/chart-of-accounts/${accountCode}/balance`, {
  ...
  private async createChartOfAccount(accountData: any): Promise<any> {
    try {
      const response = await firstValueFrom(
        this.httpService.post('/accounting/chart-of-accounts', accountData)
      );
  ...
  private performComplianceCheck(account: GhanaAccountMapping, transactionType: string): boolean {
  ...
  private validateVATCompliance(account: GhanaAccountMapping, transactionType: string): boolean {
    // Implement VAT compliance validation
    return true;
  }

  private validatePetroleumTaxCompliance(account: GhanaAccountMapping, transactionType: string): boolean {
