...
export interface NPAPermitValidation {
  ...
  volumeLimits: { [key in ProductGrade]?: number };
  validationDetails: {
...
export interface EnvironmentalAssessment {
  ...
  limits: { [parameter: string]: number };
...
export interface QualityTestResult {
  ...
  parameterResults: { [parameter: string]: number };
...
@Injectable()
export class GhanaComplianceService {
  private readonly logger = new Logger(GhanaComplianceService.name);
   ...
   */
  async validateDeliveryCompliance(delivery: DailyDelivery): Promise<GhanaComplianceValidationResult> {
    ...
    const correctionRequired: string[] = [];
    ...
    try {
      ...
      const result: GhanaComplianceValidationResult = {
        ...
        warnings,
        recommendations,
        nextValidationDate: addDays(new Date(), 90), // Quarterly validation
        certificationStatus,
      };
   ...
   * Validate NPA permit
   */
  async validateNPAPermit(permitNumber: string, productType?: ProductGrade, quantity?: number): Promise<NPAPermitValidation> {
    try {
      this.logger.log(`Validating NPA permit: ${permitNumber}`);

      // Call NPA validation service
      const response = await firstValueFrom(
        this.httpService.get(`/ghana/npa/permits/${permitNumber}/validate`, {
          params: {
            productType,
            quantity,
      ...
      const validationDetails = {
        ...
        permitNotExpired: permitData.expiryDate ? isAfter(new Date(permitData.expiryDate), new Date()) : false,
      ...
      if (!validationDetails.permitExists) errors.push('Permit not found in NPA database');
      if (!validationDetails.permitActive) errors.push('Permit is not active');
      if (!validationDetails.permitNotExpired) errors.push('Permit has expired');
      if (!validationDetails.holderVerified) errors.push('Permit holder verification failed');
      if (!validationDetails.productAuthorized) errors.push(`Product ${productType} not authorized on permit`);
      if (!validationDetails.volumeWithinLimits) errors.push('Quantity exceeds permit volume limits');

      // Generate warnings
      if (permitData.expiryDate && differenceInDays(new Date(permitData.expiryDate), new Date()) <= 30) {
   ...
   */
  async validateCustomsEntry(entryNumber: string, productType?: ProductGrade, quantity?: number): Promise<CustomsEntryValidation> {
    try {
      this.logger.log(`Validating customs entry: ${entryNumber}`);

      // Call Ghana Customs validation service
      const response = await firstValueFrom(
        this.httpService.get(`/ghana/customs/entries/${entryNumber}/validate`, {
          params: {
            productType,
            quantity,
            validationDate: new Date().toISOString(),
      ...
      if (!validationDetails.entryExists) errors.push('Customs entry not found');
      if (!validationDetails.clearanceCompleted) errors.push('Customs clearance not completed');
      if (!validationDetails.dutiesPaid) errors.push('Duties and taxes not fully paid');
      if (!validationDetails.documentsComplete) errors.push('Required customs documents incomplete');
      if (!validationDetails.quantityMatches) warnings.push('Declared quantity differs from delivery quantity');
      if (!validationDetails.productClassificationCorrect) errors.push('Product classification mismatch');
   ...
   */
  async validateTaxCalculations(delivery: DailyDelivery): Promise<TaxCalculationValidation> {
    const calculationDetails: TaxCalculationDetail[] = [];
    const discrepancies: TaxDiscrepancy[] = [];
    ...
    for (const validation of taxValidations) {
      const calculatedAmount = new Decimal(validation.taxableAmount).mul(validation.rate).toNumber();
      const variance = new Decimal(validation.actualAmount).minus(calculatedAmount).toNumber();
      ...
      const detail: TaxCalculationDetail = {
        taxType: validation.type as any,
   ...
   */
  async validateEnvironmentalCompliance(delivery: DailyDelivery): Promise<EnvironmentalCompliance> {
    try {
      const response = await firstValueFrom(
        this.httpService.get(`/ghana/environmental/compliance/${delivery.productType}`, {
          params: {
   ...
   */
  async validateQualityStandards(delivery: DailyDelivery): Promise<QualityStandardCompliance> {
   ...
   */
  async checkUPPFEligibility(customerId: string, productType: ProductGrade, quantity: number): Promise<UPPFEligibilityCheck> {
    try {
      const response = await firstValueFrom(
        this.httpService.get(`/ghana/uppf/eligibility/${customerId}`, {
          params: {
            productType,
            quantity,
          },
  ...
  private checkVolumeLimit(volumeLimits: { [key in ProductGrade]?: number }, productType?: ProductGrade, quantity?: number): boolean {
    if (!productType || !quantity || !volumeLimits) return true;
    const limit = volumeLimits[productType];
    return !limit || quantity <= limit;
  }

  private validateQuantityMatch(declaredQuantity?: number, actualQuantity?: number): boolean {
    if (!declaredQuantity || !actualQuantity) return true;
    const tolerance = 0.05; // 5% tolerance
    const variance = Math.abs(declaredQuantity - actualQuantity) / declaredQuantity;
    return variance <= tolerance;
  }

  private validateProductClassification(declaredProductCode?: string, actualProductType?: ProductGrade): boolean {
    if (!declaredProductCode || !actualProductType) return true;
  ...
  private async getApplicableQualityStandards(productType: ProductGrade): Promise<QualityStandard[]> {
    const ghanaStandards: QualityStandard[] = [
      {
        standardId: 'GS_304',
  ...
  private mapNPAValidationToCheckResult(npaResult: NPAPermitValidation): ComplianceCheckResult {
    return {
      checkType: 'NPA_PERMIT',
      checkName: 'NPA Permit Validation',
  ...
  private mapCustomsValidationToCheckResult(customsResult: CustomsEntryValidation): ComplianceCheckResult {
    return {
      checkType: 'CUSTOMS_ENTRY',
      checkName: 'Customs Entry Validation',
  ...
  private mapTaxValidationToCheckResult(taxResult: TaxCalculationValidation): ComplianceCheckResult {
    return {
  ...
  private mapEnvironmentalToCheckResult(envResult: EnvironmentalCompliance): ComplianceCheckResult {
    return {
  ...
  private mapQualityToCheckResult(qualityResult: QualityStandardCompliance): ComplianceCheckResult {
    return {
  ...
  private mapUPPFToCheckResult(uppfResult: UPPFEligibilityCheck): ComplianceCheckResult {
    return {
  ...
  private assessCertificationStatus(validationResults: ComplianceCheckResult[]): CertificationStatus {
    ...
    let overallStatus: 'VALID' | 'EXPIRED' | 'EXPIRING_SOON' | 'INVALID';
    
    if (npaValid && customsValid && envValid && qualityValid) {
      // Check if any certificates are expiring soon
      const hasExpiringCerts = validationResults.some(r => 
        r.validUntil && differenceInDays(r.validUntil, new Date()) <= 30
      );
  ...
  private generateComplianceRecommendations(validationResults: ComplianceCheckResult[]): string[] {
    ...
    for (const result of validationResults) {
      ...
      if (result.validUntil && differenceInDays(result.validUntil, new Date()) <= 60) {
