...
import { Repository, DataSource, QueryRunner, In, Between, Like, IsNull, Not } from 'typeorm';
import { EventEmitter2 } from '@nestjs/event-emitter';
import { DailyDelivery, DeliveryStatus, StationType, RevenueRecognitionType } from '../entities/daily-delivery.entity';
import { DeliveryLineItem } from '../entities/delivery-line-item.entity';
import { DeliveryApprovalHistory, ApprovalAction } from '../entities/delivery-approval-history.entity';
...
export class DailyDeliveryService {
  private readonly logger = new Logger(DailyDeliveryService.name);
  ...
  constructor(
    ...
    @InjectRepository(DeliveryDocuments)
    private readonly documentsRepository: Repository<DeliveryDocuments>,
  ...
  async create(createDeliveryDto: CreateDailyDeliveryDto): Promise<DailyDelivery> {
    ...
    try {
      ...
      if (existingPsa) {
        throw new ConflictException(`PSA number ${createDeliveryDto.psaNumber} already exists`);
      }
      ...
      if (existingWaybill) {
        throw new ConflictException(`Waybill number ${createDeliveryDto.waybillNumber} already exists`);
      }

      // Get price build-up components if station type provided
      let priceBuildupComponents = null;
      let revenueRecognitionType = createDeliveryDto.revenueRecognitionType || RevenueRecognitionType.IMMEDIATE;
      
      if (createDeliveryDto.stationType) {
        // Auto-determine revenue recognition based on station type
        if (createDeliveryDto.stationType === StationType.COCO || createDeliveryDto.stationType === StationType.DOCO) {
          revenueRecognitionType = RevenueRecognitionType.DEFERRED;
        }

        // Fetch price build-up components from pricing service
        try {
          priceBuildupComponents = await this.fetchPriceBuildupComponents(
            createDeliveryDto.productType,
            createDeliveryDto.stationType,
      ...
      if (createDeliveryDto.lineItems && createDeliveryDto.lineItems.length > 0) {
        ...
        await queryRunner.manager.save(DeliveryLineItem, lineItems);
      }
  ...
  async findAll(query: QueryDailyDeliveryDto, tenantId: string): Promise<{ deliveries: DailyDelivery[], total: number, page: number, limit: number }> {
    ...
    if (query.search) {
      queryBuilder.andWhere(
        '(delivery.deliveryNumber ILIKE :search OR delivery.customerName ILIKE :search OR delivery.vehicleRegistrationNumber ILIKE :search OR delivery.psaNumber ILIKE :search OR delivery.waybillNumber ILIKE :search)',
        { search: `%${query.search}%` }
      );
    }
    ...
    if (query.pendingApprovalsOnly) {
      queryBuilder.andWhere('delivery.status = :pendingStatus', { pendingStatus: DeliveryStatus.PENDING_APPROVAL });
    }
  ...
  async findOne(id: string, tenantId: string): Promise<DailyDelivery> {
  ...
  async update(id: string, updateDeliveryDto: UpdateDailyDeliveryDto, tenantId: string): Promise<DailyDelivery> {
    const delivery = await this.findOne(id, tenantId);

    // Check if delivery can be updated
    if (delivery.status === DeliveryStatus.COMPLETED || delivery.status === DeliveryStatus.CANCELLED) {
      throw new BadRequestException('Cannot update completed or cancelled delivery');
    }
  ...
  async delete(id: string, tenantId: string): Promise<void> {
    ...
    if (delivery.status !== DeliveryStatus.DRAFT && delivery.status !== DeliveryStatus.CANCELLED) {
  ...
  async submitForApproval(id: string, submitDto: SubmitForApprovalDto, tenantId: string): Promise<DailyDelivery> {
    const delivery = await this.findOne(id, tenantId);

    if (delivery.status !== DeliveryStatus.DRAFT) {
      throw new BadRequestException('Only draft deliveries can be submitted for approval');
    }

    // Validate for approval
    const validation = await this.validationService.validateForApproval(delivery);
    if (!validation.isValid) {
      throw new BadRequestException(`Cannot submit for approval: ${validation.errors.join(', ')}`);
    }
    ...
    try {
      ...
      await queryRunner.manager.update(DailyDelivery, id, {
  ...
  async processApproval(id: string, processDto: ProcessApprovalDto, tenantId: string): Promise<DailyDelivery> {
    const delivery = await this.findOne(id, tenantId);

    if (delivery.status !== DeliveryStatus.PENDING_APPROVAL) {
      throw new BadRequestException('Only pending approval deliveries can be processed');
    }
    ...
    try {
      ...
      switch (processDto.action) {
        case ApprovalAction.APPROVED:
          newStatus = DeliveryStatus.APPROVED;
          approvalData = {
            approvedBy: processDto.approvedBy,
        ...
        case ApprovalAction.RETURNED:
          newStatus = DeliveryStatus.DRAFT;
          break;
        case ApprovalAction.CANCELLED:
  ...
  async markInTransit(id: string, userId: string, tenantId: string): Promise<DailyDelivery> {
    const delivery = await this.findOne(id, tenantId);

    if (delivery.status !== DeliveryStatus.APPROVED) {
      throw new BadRequestException('Only approved deliveries can be marked as in transit');
    }
  ...
  async markDelivered(id: string, userId: string, tenantId: string, actualDeliveryTime?: Date): Promise<DailyDelivery> {
    const delivery = await this.findOne(id, tenantId);
  ...
  async addDocument(createDocumentDto: CreateDeliveryDocumentDto): Promise<DeliveryDocuments> {
  ...
  async verifyDocument(documentId: string, verifyDto: VerifyDocumentDto): Promise<DeliveryDocuments> {
  ...
  async getDeliveryStats(tenantId: string, fromDate?: string, toDate?: string): Promise<any> {
    const queryBuilder = this.deliveryRepository.createQueryBuilder('delivery');
    queryBuilder.where('delivery.tenantId = :tenantId', { tenantId });
    ...
    const stats = await queryBuilder
      ...
      .setParameters({
        ...
        pending: DeliveryStatus.PENDING_APPROVAL,
      })
      .getRawOne();
   ...
   * Fetch price build-up components from pricing service
   */
  private async fetchPriceBuildupComponents(productType: string, stationType: StationType, deliveryDate: string): Promise<any> {
    ...
    return {
      ...
      dealerMargin: stationType === StationType.DODO ? 0.15 : 0,
      ...
      levies: {
        ...
        retailMargin: stationType === StationType.DODO ? 0.18 : 0
      },
   ...
   */
  async createInventoryMovement(delivery: DailyDelivery, userId: string): Promise<void> {
    if (!delivery.requiresInventoryMovement()) {
      throw new BadRequestException('Delivery does not require inventory movement');
    }
   ...
   */
  async createImmediateSale(delivery: DailyDelivery, userId: string): Promise<void> {
    if (!delivery.requiresImmediateSale()) {
      throw new BadRequestException('Delivery does not require immediate sale processing');
    }
   ...
   */
  async processDeferredRevenueRecognition(delivery: DailyDelivery, userId: string): Promise<void> {
    if (!delivery.shouldDeferRevenue()) {
      throw new BadRequestException('Delivery does not require deferred revenue recognition');
    }
   ...
   */
  async completeDeliveryWithStationTypeLogic(id: string, userId: string, tenantId: string): Promise<DailyDelivery> {
    const delivery = await this.findOne(id, tenantId);
