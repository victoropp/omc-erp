...
describe('ConfigurationService', () => {
    ...
    const mockConfiguration = {
        ...
        encryptedValue: null,
        defaultValue: 'USD',
        module: configuration_entity_1.ConfigurationModule.SYSTEM,
        type: configuration_entity_1.ConfigurationType.OPERATIONAL,
    ...
    beforeEach(async () => {
        ...
        const module = await testing_1.Test.createTestingModule({
            providers: [
                ...
                {
                    provide: (0, typeorm_1.getRepositoryToken)(configuration_entity_1.Configuration),
                    useValue: {
                        findOne: jest.fn(),
                        find: jest.fn(),
                        create: jest.fn(),
        ...
        service = module.get(configuration_service_1.ConfigurationService);
        configRepository = module.get((0, typeorm_1.getRepositoryToken)(configuration_entity_1.Configuration));
        ...
        mockCrypto.createCipher = jest.fn(() => mockCipher);
        mockCrypto.createDecipher = jest.fn(() => mockDecipher);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe('getConfiguration', () => {
        ...
        it('should fetch from database if not cached', async () => {
            ...
            const getHierarchySpy = jest.spyOn(service, 'getConfigurationHierarchy')
                .mockResolvedValue(hierarchyConfigs);
            const resolveValueSpy = jest.spyOn(service, 'resolveConfigurationValue')
                .mockReturnValue('GHS');
            // Act
            const result = await service.getConfiguration(configKey, mockTenantId);
            // Assert
        ...
        it('should increment access count', async () => {
            ...
            expect(configRepository.increment).toHaveBeenCalledWith({ key: configKey, tenantId: mockTenantId, module: undefined }, 'accessCount', 1);
        });
    });
    describe('getMultipleConfigurations', () => {
        ...
        it('should filter by module when provided', async () => {
            // Arrange
            const module = configuration_entity_1.ConfigurationModule.SYSTEM;
            configRepository.find.mockResolvedValue([mockConfiguration]);
            jest.spyOn(service, 'resolveConfigurationValue').mockReturnValue('GHS');
            // Act
    ...
    describe('createConfiguration', () => {
        ...
        it('should create configuration successfully', async () => {
            ...
            const clearCacheSpy = jest.spyOn(service, 'clearConfigurationCache').mockImplementation();
            // Act
        ...
        it('should encrypt sensitive values', async () => {
            ...
            jest.spyOn(service, 'encryptValue').mockReturnValue('encrypted_secret_value');
            // Act
    ...
    describe('updateConfiguration', () => {
        ...
        it('should update configuration successfully', async () => {
            ...
            jest.spyOn(service, 'validateConfigurationValue').mockImplementation();
            jest.spyOn(service, 'clearConfigurationCache').mockImplementation();
            // Act
        ...
        it('should encrypt sensitive values on update', async () => {
            ...
            const encryptSpy = jest.spyOn(service, 'encryptValue').mockReturnValue('new_encrypted_value');
        ...
        it('should refresh dependent configurations', async () => {
            ...
            const refreshDependentsSpy = jest.spyOn(service, 'refreshDependentConfigurations').mockImplementation();
            // Act
    ...
    describe('deleteConfiguration', () => {
        it('should soft delete configuration', async () => {
            // Arrange
            configRepository.findOne.mockResolvedValue(mockConfiguration);
            const deletedConfig = { ...mockConfiguration, isActive: false, status: configuration_entity_1.ConfigurationStatus.ARCHIVED };
            ...
            expect(deletedConfig.status).toBe(configuration_entity_1.ConfigurationStatus.ARCHIVED);
    ...
    describe('isFeatureEnabled', () => {
        ...
        it('should return true for enabled feature flag', async () => {
            ...
            jest.spyOn(service, 'getConfigurationHierarchy').mockResolvedValue([featureConfig]);
            // Act
        ...
        it('should return false if feature flag config not found', async () => {
            // Arrange
            jest.spyOn(service, 'getConfigurationHierarchy').mockResolvedValue([]);
            // Act
            const result = await service.isFeatureEnabled(featureKey, mockTenantId);
            // Assert
            expect(result).toBe(false);
        ...
        it('should handle gradual rollout based on percentage', async () => {
            ...
            mockCrypto.createHash = jest.fn(() => mockHash);
            // Act
    ...
    describe('resolveConfigurationValue', () => {
        ...
        it('should skip inactive configurations', () => {
            ...
            const inactiveConfig = {
                ...mockTenantConfig,
                inheritanceLevel: 2,
                status: configuration_entity_1.ConfigurationStatus.INACTIVE,
                isActive: false,
    ...
    describe('Error Handling and Edge Cases', () => {
        ...
        it('should handle cache errors gracefully', async () => {
            ...
            jest.spyOn(service, 'getConfigurationHierarchy').mockResolvedValue([mockConfiguration]);
            jest.spyOn(service, 'resolveConfigurationValue').mockReturnValue('value');
    ...
    describe('Performance Tests', () => {
        ...
        it('should handle concurrent configuration access', async () => {
            ...
            mockCache.get.mockReturnValue(undefined);
            jest.spyOn(service, 'getConfigurationHierarchy').mockResolvedValue([mockConfiguration]);
            jest.spyOn(service, 'resolveConfigurationValue').mockReturnValue('test_value');
            const concurrentRequests = Array(20).fill(null).map(() => service.getConfiguration(configKey, mockTenantId));
            // Act
