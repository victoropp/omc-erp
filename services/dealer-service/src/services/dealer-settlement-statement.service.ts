...
export interface StatementTemplate {
  ...
  language: 'EN' | 'FR'; // English or French for Ghana
...
@Injectable()
export class DealerSettlementStatementService {
  private readonly logger = new Logger(DealerSettlementStatementService.name);
  ...
  async generateSettlementStatement(
    settlementId: string,
    tenantId: string,
    template: StatementTemplate = { templateType: 'DETAILED', format: 'PDF', language: 'EN', includeCharts: true, includeLoanDetails: true, includePerformanceMetrics: true },
    ...
    // Build statement
    const statement: SettlementStatementDto = {
      companyInfo,
      dealerInfo,
  ...
  async generateBatchStatements(
    ...
    tenantId: string,
    template: StatementTemplate = { templateType: 'STANDARD', format: 'PDF', language: 'EN', includeCharts: false, includeLoanDetails: true, includePerformanceMetrics: false },
  ): Promise<{
    ...
    const statements: SettlementStatementDto[] = [];
  ...
  async generatePDFStatement(
    statement: SettlementStatementDto,
...
<body>
  ...
  private buildStatementDetails(settlement: DealerSettlement): any {
    return {
  ...
  private buildSalesSummary(settlement: DealerSettlement): any {
    const calculationDetails = settlement.calculationDetails;
    const totalLitres = settlement.totalLitresSold;
    
    const periodDays = Math.ceil(
      (settlement.periodEnd.getTime() - settlement.periodStart.getTime()) / (24 * 60 * 60 * 1000)
    );

    // Build product breakdown from calculation details
    const productBreakdown = Object.entries(calculationDetails?.salesByProduct || {}).map(([product, data]: [string, any]) => {
  ...
  private buildPriceBuildUp(settlement: DealerSettlement): any {
  ...
  private buildMarginCalculation(settlement: DealerSettlement): any {
  ...
  private buildDeductionsDetail(settlement: DealerSettlement, activeLoans: DealerLoan[]): any {
  ...
  private buildSettlementSummary(settlement: DealerSettlement): any {
  ...
  private buildPaymentInstructions(dealerInfo: any, settlement: DealerSettlement): any {
    return {
      bankDetails: {
  ...
  private buildPerformanceMetrics(settlement: DealerSettlement, previousSettlement: DealerSettlement | null): any {
  ...
  private buildLoanSummary(activeLoans: DealerLoan[]): any {
    if (activeLoans.length === 0) return undefined;
    ...
    let loanPerformance: 'GOOD' | 'SATISFACTORY' | 'POOR' = 'GOOD';
    if (averageDaysOverdue > 30) loanPerformance = 'POOR';
    else if (averageDaysOverdue > 7) loanPerformance = 'SATISFACTORY';
  ...
  private buildComplianceInfo(settlement: DealerSettlement): any {
    return {
  ...
  private async getDealerInfo(stationId: string, tenantId: string): Promise<any> {
  ...
  private async getCompanyInfo(tenantId: string): Promise<any> {
  ...
  private async getPreviousSettlement(stationId: string, currentWindowId: string, tenantId: string): Promise<DealerSettlement | null> {
  ...
  private getStatementCSS(): string {
    return `
